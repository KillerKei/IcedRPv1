...
function bankBalance(player)
  return irpCore.GetPlayerFromId(player).getBank()
end
...
function deposit(player, amount)
  ...
  local user = irpCore.GetPlayerFromId(player)
...
AddEventHandler('bank:transfer', function(to, amountt)
	...
	local zPlayer = irpCore.GetPlayerFromId(to)
	local balance = 0
...
TriggerEvent('es:addCommand', 'givecash', function(source, args)
  ...
  local amount = args[2]
  local user = irpCore.GetPlayerFromId(sender)


  local playerRec = irpCore.GetPlayerFromId(reciever)
...
AddEventHandler("cash:remove", function(user, amount)
  local _source = source
  local xPlayer = irpCore.GetPlayerFromId(user)
