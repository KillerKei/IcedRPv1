...
Citizen.CreateThread(function()
	...
	while true do
			...
			if dstscan < 2.5 then
				...
				if IsPedInAnyVehicle(PlayerPedId(), true) == false and dstscan < 0.25 then
					...
					if IsControlJustPressed(1, 38) then	
						irpCore.TriggerServerCallback('irp-license:checkLicense', function(hasWeaponLicense)
		...
		if(Vdist( 1112.52, -2000.86, 30.96, pos.x, pos.y, pos.z) < 20.0)then
			found = true
			DrawMarker(27,  1112.52, -2000.86, 30.96 - 1, 0, 0, 0, 0, 0, 0, 1.0001, 1.0001, 1.5001, 0, 25, 165, 165, 0,0, 0,0)
			if(Vdist( 1112.52, -2000.86, 30.96, pos.x, pos.y, pos.z) < 2.0)then
				DisplayHelpText("Press ~INPUT_CONTEXT~ to open the ~g~smelter.")
				if IsControlJustPressed(1, 38) then	
					local finished = exports["irp-taskbar"]:taskBar(60000,"Readying Smelter")
    	...
    	if(Vdist(methlocation["x"],methlocation["y"],methlocation["z"], pos.x, pos.y, pos.z) < 10.0)then
			...
			if(Vdist(methlocation["x"],methlocation["y"],methlocation["z"], pos.x, pos.y, pos.z) < 5.0)then
				...
				if IsControlJustPressed(1, 38) then
					local finished = exports["irp-taskbar"]:taskBar(60000,"Searching...")
		...
		-- 			local finished = exports["irp-taskbar"]:taskBar(60000,"Making a god slushy...")
      	-- 			if (finished == 100) then
		...
		if(Vdist(-196.7726, -1319.347, 31.08931, pos.x, pos.y, pos.z) < 20.0)then
			found = true
			DrawMarker(27, -196.7726, -1319.347, 31.08931 - 1, 0, 0, 0, 0, 0, 0, 1.0001, 1.0001, 1.5001, 0, 25, 165, 165, 0,0, 0,0)
			if(Vdist(-196.7726, -1319.347, 31.08931, pos.x, pos.y, pos.z) < 2.0)then
				local job = irpCore.GetPlayerData().job.name
				DisplayHelpText("Press ~INPUT_CONTEXT~ to open the ~g~shop.")
				if IsControlJustPressed(1, 38) then	
					if job == 'mecano' then
						TriggerEvent("server-inventory-open", "210", "Craft");	
						Wait(1000)
						else
						TriggerEvent('DoLongHudText', "You Need To Be Mechanic!", 2)
					end
			    end
			end
		end

		if(Vdist(-320.202, -131.9038, 38.97113, pos.x, pos.y, pos.z) < 20.0)then
			found = true
			DrawMarker(27, -320.202, -131.9038, 38.97113 - 1, 0, 0, 0, 0, 0, 0, 1.0001, 1.0001, 1.5001, 0, 25, 165, 165, 0,0, 0,0)
			if(Vdist(-320.202, -131.9038, 38.97113, pos.x, pos.y, pos.z) < 2.0)then
				local job = irpCore.GetPlayerData().job.name
				DisplayHelpText("Press ~INPUT_CONTEXT~ to open the ~g~shop.")
				if IsControlJustPressed(1, 38) then	
					if job == 'mecano' then
