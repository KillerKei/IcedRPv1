...
RegisterNetEvent('irp-garages:client:houseGarageConfig')
AddEventHandler('irp-garages:client:houseGarageConfig', function(garageConfig)
...
AddEventHandler('playerSpawned', function(spawn)
	if not hasAlreadyJoined then
		TriggerServerEvent('irp-garage:putcaringarage')
	end
...
function RentrerVehicule()
	...
	--TriggerServerEvent('irp-garage:CheckForVeh')
...
function ListeVehicule(page)
	...
	ClearMenu()
	local carCount = 0
	irpCore.TriggerServerCallback('irp-garage:getOwnedCars', function(ownedCars)
		   ...
		   local vehicle_state = value.state
			...
			--if garages[selectedGarage].garage == curGarage then
					...
					if vehicle_state == "Standard Impound" then
						Menu.addButton(tostring(vehicleName), "OptionVehicle", value.id, "Impounded: $500", " Engine %:" .. irpCore.Math.Round(enginePercent,1) .. "", " Body %:" .. irpCore.Math.Round(bodyPercent,1) .. "", "Plate: "..plate.."")
					elseif vehicle_state == "Police Impound" then
						Menu.addButton(tostring(vehicleName), "OptionVehicle", value.id, "Impounded: $1500", " Engine %:" .. irpCore.Math.Round(enginePercent,1) .. ""," Body %:" .. irpCore.Math.Round(bodyPercent,1) .. "", "Plate: "..plate.."")
					elseif curGarage ~= current_used_garage and curGarage ~= "Any" and vehicle_state ~= "Out" and curGarage ~= "house" then
						Menu.addButton(tostring(vehicleName), "OptionVehicle", 0, curGarage, " Engine %:" .. irpCore.Math.Round(enginePercent,1) .. "", " Body %:" .. irpCore.Math.Round(bodyPercent,1) .. "", "Plate: "..plate.."")
					--elseif currentGarage == ""
					else

						Menu.addButton(tostring(vehicleName), "OptionVehicle", value.id, tostring(value.state) , " Engine %:" .. irpCore.Math.Round(enginePercent,1) .. ""," Body %:" .. irpCore.Math.Round(bodyPercent,1) .. "", "Plate: "..plate.."")
					end
				end
			--end
		   count = count + 1
...
function SpawnVehicle(vehicle, plate, fuelSet, spawnx, spawny, spawx, spawnz)
	...
	irpCore.Game.SpawnVehicle(vehicle.model, {
		x = spawnx,
		y = spawny,
		z = spawx
		...
		--irpCore.Game.SetVehicleProperties(callback_vehicle, vehicle)
	...
	TriggerServerEvent('irp-advancedgarage:setVehicleState', plate, false)
end
...
SetVehicleProperties = function(vehicle, vehicleProps, fuelSet)
    irpCore.Game.SetVehicleProperties(vehicle, vehicleProps)
