    ...
    Citizen.CreateThread(function()
        ...
        while isPending do
            ...
            if count >= Config.Timeout and isPending then
                ...
                TriggerServerEvent('irp-ping:server:SendPingResult', pendingPing.id, 'timeout')
                pendingPing = nil
                isPending = false
            end
            Citizen.Wait(1000)
...
RegisterNetEvent('irp-ping:client:SendPing')
AddEventHandler('irp-ping:client:SendPing', function(sender, senderId)
    if pendingPing == nil then
        ...
        TriggerServerEvent('irp-ping:server:SendPingResult', pendingPing.id, 'received')
        ...
        TriggerServerEvent('irp-ping:server:SendPingResult', senderId, 'unable')
    end
end)

RegisterNetEvent('irp-ping:client:AcceptPing')
AddEventHandler('irp-ping:client:AcceptPing', function()
    if isPending then
        ...
        TriggerServerEvent('irp-ping:server:SendPingResult', pendingPing.id, 'accept')
        isPending = false
    else
...
RegisterNetEvent('irp-ping:client:RejectPing')
AddEventHandler('irp-ping:client:RejectPing', function()
    if pendingPing ~= nil then
        TriggerEvent('DoLongHudText', 'Rejected Ping From ' .. pendingPing.name, 2)
        TriggerServerEvent('irp-ping:server:SendPingResult', pendingPing.id, 'reject')
        pendingPing = nil
        isPending = false
    else
        TriggerEvent('DoLongHudText', 'You Have No Pending Ping', 2)
...
RegisterNetEvent('irp-ping:client:RemovePing')
AddEventHandler('irp-ping:client:RemovePing', function()
    if pendingPing ~= nil then
        RemoveBlip(pendingPing.blip)
        pendingPing = nil
