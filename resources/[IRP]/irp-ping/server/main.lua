...
RegisterCommand('ping', function(source, args, rawCommand)
	...
	if args[1] ~= nil then
        if args[1]:lower() == 'accept' then
            TriggerClientEvent('irp-ping:client:AcceptPing', source)
        elseif args[1]:lower() == 'reject' then
            TriggerClientEvent('irp-ping:client:RejectPing', source)
        else
            local tSrc = tonumber(args[1])
            if source ~= tSrc then
                TriggerClientEvent('irp-ping:client:SendPing', tSrc, name.firstname .. ' ' .. name.lastname, source)
            else
...
RegisterServerEvent('irp-ping:server:SendPingResult')
AddEventHandler('irp-ping:server:SendPingResult', function(id, result)
    local name = getIdentity(source)
