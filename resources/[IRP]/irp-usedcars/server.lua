...
local RSC = irpCore.RegisterServerCallback
...
RSC('vehsales:TryBuy',function(source,cb,veh)
  ...
  while not xPlayer do xPlayer = irpCore.GetPlayerFromId(source); Citizen.Wait(0); end
...
function DoBuy(source,veh)
  ...
  if vData then
    ...
    if vData.owner ~= identifier then
      local xPlayer = irpCore.GetPlayerFromIdentifier(vData.owner)
      local tick = 0
      while not xPlayer and tick < 1000 do
        tick = tick + 1
        xPlayer = irpCore.GetPlayerFromIdentifier(vData.owner)
        Citizen.Wait(0)
      end

      if xPlayer then 
        xPlayer.addMoney(vData.price)
        TriggerClientEvent("banking:addBalance", vData.price)
        xPlayer = nil
        local xPlayer = irpCore.GetPlayerFromId(source)
        while not xPlayer do xPlayer = irpCore.GetPlayerFromId(source); Citizen.Wait(0); end
        xPlayer.removeMoney(vData.price)
        TriggerClientEvent("banking:removeBalance", vData.price)
