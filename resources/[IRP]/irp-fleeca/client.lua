...
RegisterNetEvent("irp-fleeca:openDoor_c")
AddEventHandler("irp-fleeca:openDoor_c", function(coords, method)
    if method == 1 then
...
RegisterNetEvent("irp-fleeca:resetDoorState")
AddEventHandler("irp-fleeca:resetDoorState", function(name)
    Freeze[name] = 0
end)

RegisterNetEvent("irp-fleeca:lootup_c")
AddEventHandler("irp-fleeca:lootup_c", function(var, var2)
    LootCheck[var][var2] = true
end)

RegisterNetEvent("irp-fleeca:outcome")
AddEventHandler("irp-fleeca:outcome", function(oc, arg)
    for i = 1, #Check, 1 do
        Check[i] = false
    end
    ...
    if oc then
        Check[arg] = true
        TriggerEvent("irp-fleeca:startheist", fleeca.Banks[arg], arg)
    elseif not oc then
        TriggerEvent("DoLongHudText", arg, 2)
    end
end)

RegisterNetEvent("irp-fleeca:startLoot_c")
AddEventHandler("irp-fleeca:startLoot_c", function(data, name)
    --local check = true
    --[[while check do
                ...
                if dst < 40 then
                    if not LootCheck[name].Loot1 then
                        ...
                        if dst1 < 5 then
                            ...
                            if dst1 < 0.75 and IsControlJustReleased(0, 38) then
                                TriggerServerEvent("irp-fleeca:lootup", name, "Loot1")
                                StartGrab(name)
                            end
                        end
                    end
                    ...
                    if not LootCheck[name].Loot2 then
                        ...
                        if dst1 < 5 then
                            ...
                            if dst1 < 1 and IsControlJustReleased(0, 38) then
                                TriggerServerEvent("irp-fleeca:lootup", name, "Loot2")
                                StartGrab(name)
                            end
                        end
                    end
                    ...
                    if not LootCheck[name].Loot3 then
                        ...
                        if dst1 < 5 then
                            ...
                            if dst1 < 1 and IsControlJustReleased(0, 38) then
                                TriggerServerEvent("irp-fleeca:lootup", name, "Loot3")
                                StartGrab(name)
                            end
                        end
                    end
                    ...
                    if LootCheck[name].Stop or (LootCheck[name].Loot1 and LootCheck[name].Loot2 and LootCheck[name].Loot3) then
                        ...
                        if initiator then
                            TriggerEvent("irp-fleeca:reset", name, data)
                            return
                        end
                        return
                    end
...
RegisterNetEvent("irp-fleeca:stopHeist_c")
AddEventHandler("irp-fleeca:stopHeist_c", function(name)
    LootCheck[name].Stop = true
end)

RegisterNetEvent("irp-fleeca:policenotify")
AddEventHandler("irp-fleeca:policenotify", function(name)
    TriggerEvent('irp-dispatch:bankrobbery')
...
AddEventHandler("irp-fleeca:freezeDoors", function()
    Citizen.CreateThread(function()
        while true do
            local pedcoords = GetEntityCoords(PlayerPedId())
            ...
            for k, v in pairs(fleeca.Banks) do
                if Freeze[k] < 3 then
                    ...
                    if dst < 10 then
                        local obj = irpCore.Game.GetClosestObject(fleeca.door, vector3(v.doors.secondloc.x, v.doors.secondloc.y, v.doors.secondloc.z))
...
AddEventHandler("irp-fleeca:reset", function(name, data)
    ...
    if data.hash == nil then
        TriggerServerEvent("irp-fleeca:openDoor", vector3(data.doors.startloc.x, data.doors.startloc.y, data.doors.startloc.z), 4)
    elseif not data.hash == nil then
        TriggerServerEvent("irp-fleeca:openDoor", vector3(data.doors.startloc.x, data.doors.startloc.y, data.doors.startloc.z), 5)
    end
    TriggerEvent("irp-fleeca:cleanUp", data, name)
end)

AddEventHandler("irp-fleeca:startheist", function(data, name)
    disableinput = true
    currentname = name
    currentcoords = vector3(data.doors.startloc.x, data.doors.startloc.y, data.doors.startloc.z)
    initiator = true
    ...
    if data.hash == nil then
    TriggerServerEvent("irp-fleeca:openDoor", vector3(data.doors.startloc.x, data.doors.startloc.y, data.doors.startloc.z), 1)
    elseif data.hash ~= nil then
        TriggerServerEvent("irp-fleeca:openDoor", "anana", 2)
    end
    startdstcheck = true
    disableinput = false
    currentname = name
...
AddEventHandler("irp-fleeca:cleanUp", function(data, name)
    Citizen.Wait(60000)
    ...
    TriggerServerEvent("irp-fleeca:setCooldown", name)
    initiator = false
end)
...
function SecondDoor(data)
    ...
    exports["irp-taskbar"]:taskBar(1500, "Using Malicious Card")
        ...
        disableinput = false
        exports["irp-taskbar"]:taskBar(2000, "Accessing the Panel")
        TriggerEvent('DoLongHudText', 'Access complete!', 1)
        PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
        TriggerServerEvent("irp-fleeca:openDoor", vector3(data.doors.secondloc.x, data.doors.secondloc.y, data.doors.secondloc.z), 3)
        disableinput = false
end
...
function SpawnTrolleys(data, name)
    ...
    local players, nearbyPlayer = irpCore.Game.GetPlayersInArea(GetEntityCoords(PlayerPedId()), 20.0)
    ...
    TriggerServerEvent("irp-fleeca:startLoot", data, name, missionplayers)
    done = false
end
...
function StartGrab(name)
		    ...
		    while GetGameTimer() - startedGrabbing < 37000 do
			    ...
			    if HasAnimEventFired(ped, GetHashKey("RELEASE_CASH_DESTROY")) then
                    if IsEntityVisible(grabobj) then
                        if mathfunc == 2 then
                            TriggerServerEvent("irp-fleeca:addAlum")
                        end
                        if matherino == 15 then
                            TriggerServerEvent("irp-fleeca:addLiv")
                        end
                        SetEntityVisible(grabobj, false, false)
                        TriggerServerEvent("irp-fleeca:rewardCash")
				    end
			    end
		    end
    ...
    --TriggerServerEvent("irp-fleeca:updateObj", name, NewTrolley, 2)
        ...
        if startdstcheck then
            if initiator then
                ...
                if (GetDistanceBetweenCoords(playercoord, currentcoords, true)) > 20 then
                    ...
                    startdstcheck = false
                    TriggerServerEvent("irp-fleeca:stopHeist", currentname)
                end
            end
            Citizen.Wait(1)
        else
    ...
    if not done then
        ...
        if dst < 4 then
            ...
            if dst < 0.75 and IsControlJustReleased(0 , 38) then
                if exports['irp-inventory']:hasEnoughOfItem('cb', 1) then
                        done = true
                        TriggerEvent('inventory:removeItem', 'cb', 1)
                        return SecondDoor(fleeca.Banks[currentname])
                    elseif not exports['irp-inventory']:hasEnoughOfItem('cb', 0) then
...
Citizen.CreateThread(function()
    ...
    irpCore.TriggerServerCallback("irp-fleeca:getBanks", function(result)
        fleeca.Banks = result
    end)
    TriggerEvent("irp-fleeca:freezeDoors")
    Citizen.Wait(1000)
    while true do
            ...
            if not v.onaction then
                ...
                if dst <= 5 and not Check[k] then
                    ...
                    if dst <= 1 and IsControlJustReleased(0, 38) then
                        ...
                        --local outcome = exports['irp-thermite']:startGame(20,2,8,500)
                        if exports['irp-thermite']:startGame(20,1,8,500) then
                            FreezeEntityPosition(PlayerPedId(), false)
                            TriggerServerEvent("irp-fleeca:startcheck", k)
                        else
