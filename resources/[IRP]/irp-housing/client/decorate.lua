...
RegisterNetEvent("irp-housing:client:decorate")
AddEventHandler("irp-housing:client:decorate", function()
	Citizen.Wait(500)
	if inside then 
		if hasKey then 
...
RegisterNetEvent("irp-housing:server:sethousedecorations")
AddEventHandler("irp-housing:server:sethousedecorations", function(house, decorations)
...
function LoadDecorations(house)
	if Config.Houses[house].decorations == nil or next(Config.Houses[house].decorations) == nil then
		irpCore.TriggerServerCallback('irp-housing:server:getHouseDecorations', function(result)
...
function SaveDecorations()
	if closesthouse ~= nil then
		...
		TriggerServerEvent("irp-housing:server:savedecorations", closesthouse, ObjectList)
	end
end
...
function CheckMovementInput()
	...
	if IsControlJustReleased(0, Keys["LEFTSHIFT"]) then
		...
		irpCore.ShowNotification('Speed is '.. tostring(speeds[curSpeed]))
	end
