...
local servername = 'irp Development'
local cam = nil
local viewCam = false
...
RegisterNetEvent('irp-housing:client:sellHouse')
AddEventHandler('irp-housing:client:sellHouse', function()
    if closesthouse ~= nil and hasKey then
        TriggerServerEvent('irp-housing:server:viewHouse', closesthouse)
    end
end)
...
AddEventHandler('irp:playerLoaded', function()
    TriggerServerEvent('irp-housing:client:setHouses')
    isLoggedIn = true
    SetClosestHouse()
    TriggerEvent('irp-housing:client:setupHouseBlips')
    Citizen.Wait(100)
    TriggerServerEvent("irp-housing:server:setHouses")
end)

RegisterNetEvent('irp-housing:client:setHouseConfig')
AddEventHandler('irp-housing:client:setHouseConfig', function(houseConfig)
    Config.Houses = houseConfig
    --TriggerEvent("irp-housing:client:refreshHouse")
	
end)

RegisterNetEvent('irp-housing:client:lockHouse')
AddEventHandler('irp-housing:client:lockHouse', function(bool, house)
    Config.Houses[house].locked = bool
end)

RegisterNetEvent('irp-housing:client:createHouses')
AddEventHandler('irp-housing:client:createHouses', function(price, tier)
    ...
    TriggerServerEvent('irp-housing:server:addNewHouse', area, coords, price, tier)
end)

RegisterNetEvent('irp-housing:client:toggleDoorlock')
AddEventHandler('irp-housing:client:toggleDoorlock', function()
    ...
    if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, true) < 1.5)then
        if hasKey then
            if Config.Houses[closesthouse].locked then
                TriggerServerEvent('irp-housing:server:lockHouse', false, closesthouse)
				TriggerEvent('DoLongHudText', 'The Door is unlocked.', 1)
            else
                TriggerServerEvent('irp-housing:server:lockHouse', true, closesthouse)
                    ...
                    if CurrentDoorBell ~= 0 then
                        if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[closesthouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[closesthouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z, true) < 1.5)then
                            ...
                            if IsControlJustPressed(0, Keys["G"]) then
                                TriggerServerEvent("irp-housing:server:OpenDoor", CurrentDoorBell, closesthouse)
                                CurrentDoorBell = 0
                            end
                        end
                    end
                            ...
                            if not inOwned then
                                if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, true) < 1.5)then
                                    if not Config.Houses[closesthouse].locked then
                                        ...
                                        if IsControlJustPressed(0, Keys["G"]) then
                                            TriggerServerEvent('irp-housing:server:RingDoor', closesthouse)
                                        end
                                    end
                                end
                            elseif inOwned then
                    ...
                    if closesthouse ~= nil then
                        if stashLocation ~= nil then
                            if(GetDistanceBetweenCoords(pos, stashLocation.x, stashLocation.y, stashLocation.z, true) < 1.5)then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
									TriggerEvent("server-inventory-open", "1", "house"..closesthouse)
                    ...
                    if closesthouse ~= nil then
                        if logoutLocation ~= nil then
                            if(GetDistanceBetweenCoords(pos, logoutLocation.x, logoutLocation.y, logoutLocation.z, true) < 1.5)then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    TriggerEvent('irp-login:swapcharacter')
                                    leaveOwnedHouse(closesthouse)
                                    showClothing = false
                                end
...
RegisterNetEvent('irp-housing:client:RingDoor')
AddEventHandler('irp-housing:client:RingDoor', function(player, house)
...
RegisterNetEvent('irp-housing:client:giveHouseKey')
AddEventHandler('irp-housing:client:giveHouseKey', function(data)
    ...
    if player ~= -1 and distance < 2.5 and closesthouse ~= nil then
        ...
        if housedist < 10 then
            TriggerServerEvent('irp-housing:server:giveHouseKey', playerId, closesthouse)
        else
...
RegisterNetEvent('irp-housing:client:removeHouseKey')
AddEventHandler('irp-housing:client:removeHouseKey', function(data)
    if closesthouse ~= nil then 
        local housedist = GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z)
        if housedist < 5 then
            irpCore.TriggerServerCallback('irp-housing:server:getHouseOwner', function(result)
                if irpCore.GetPlayerData().identifier == result then
                    inHoldersMenu = true
                    HouseKeysMenu()
...
RegisterNetEvent('irp-housing:client:refreshHouse')
AddEventHandler('irp-housing:client:refreshHouse', function(data)
    Citizen.Wait(100)
    SetClosestHouse()
end)

RegisterNetEvent('irp-housing:client:setstash')
AddEventHandler('irp-housing:client:setstash', function(data)
ExecuteCommand("setlocation stash")
end)

RegisterNetEvent('irp-housing:client:setstash2')
AddEventHandler('irp-housing:client:setstash2', function(data)
ExecuteCommand("setlocation stash2")
end)

RegisterNetEvent('irp-housing:client:setoutfit')
AddEventHandler('irp-housing:client:setoutfit', function(data)
ExecuteCommand("setlocation outfit")
end)

RegisterNetEvent('irp-housing:client:setlogout')
AddEventHandler('irp-housing:client:setlogout', function(data)
ExecuteCommand("setlocation logout")
end)

RegisterNetEvent('irp-housing:client:SpawnInApartment')
AddEventHandler('irp-housing:client:SpawnInApartment', function(house)
    local pos = GetEntityCoords(GetPlayerPed(-1))
...
function removeHouseKey(citizenData)
    TriggerServerEvent('irp-housing:server:removeHouseKey', closesthouse, citizenData)
    closeMenuFull()
end
...
function enterOwnedHouse(house)
    ...
    if Config.Houses[house].tier == 1 then
        data = exports['irp-interior']:CreateTier1House(coords)
	elseif Config.Houses[house].tier == 2 then
        data = exports['irp-interior']:CreateMichaelShell(coords)
	elseif Config.Houses[house].tier == 3 then
        data = exports['irp-interior']:CreateTrevorsShell(coords)
	elseif Config.Houses[house].tier == 4 then
        data = exports['irp-interior']:CreateFranklinShell(coords)
	elseif Config.Houses[house].tier == 5 then
        data = exports['irp-interior']:CreateFranklinAuntShell(coords)
	elseif Config.Houses[house].tier == 6 then
        data = exports['irp-interior']:CreateApartmentShell(coords)
	elseif Config.Houses[house].tier == 7 then
        data = exports['irp-interior']:CreateCaravanShell(coords)
    end
    Citizen.Wait(100)
    houseObj = data[1]
    POIOffsets = data[2]
    ...
    TriggerEvent('irp-weathersync:client:DisableSync')
    TriggerEvent('irp-weed:client:getHousePlants', house)
    Citizen.Wait(100)
...
RegisterNetEvent('irp-housing:client:enterOwnedHouse')
AddEventHandler('irp-housing:client:enterOwnedHouse', function(house)
	enterOwnedHouse(house)
end)

RegisterNUICallback('HasEnoughMoney', function(data, cb)
    irpCore.TriggerServerCallback('irp-housing:server:HasEnoughMoney', function(hasEnough)
        
    end, data.objectData)
end)

RegisterNetEvent('irp-housing:client:LastLocationHouse')
AddEventHandler('irp-housing:client:LastLocationHouse', function(houseId)
...
function leaveOwnedHouse(house)
    ...
    DoScreenFadeOut(250)
    Citizen.Wait(500)
    exports['irp-interior']:DespawnInterior(houseObj, function()
        UnloadDecorations()
        TriggerEvent('irp-weathersync:client:EnableSync')
        Citizen.Wait(250)
        DoScreenFadeIn(250)
        ...
        inside = false
        TriggerEvent('irp-weed:client:leaveHouse')
        showClothing = false
    end)
end
...
function enterNonOwnedHouse(house)
    ...
    Citizen.Wait(500)
    SetRainFxIntensity(0.0)
    TriggerServerEvent('irp-housing:server:SetInsideMeta', house, true)
    TriggerEvent('irp-weathersync:client:DisableSync')
    TriggerEvent('irp-weed:client:getHousePlants', house)
    Citizen.Wait(100)
    SetWeatherTypePersist('EXTRASUNNY')
...
function leaveNonOwnedHouse(house)
    ...
    exports['irp-interior']:DespawnInterior(houseObj, function()
        ...
        showClothing = false
        TriggerEvent('irp-weed:client:leaveHouse')
        TriggerServerEvent('irp-housing:server:SetInsideMeta', house, false)
    end)
end
...
function HouseKeysMenu()
    ...
    MenuTitle = "Keys"
    ClearMenu()
    irpCore.TriggerServerCallback('irp-housing:server:getHouseKeyHolders', function(holders)
        ped = GetPlayerPed(-1);
...
RegisterNetEvent('irp-housing:client:setupHouseBlips')
AddEventHandler('irp-housing:client:setupHouseBlips', function()
    Citizen.CreateThread(function()
        Citizen.Wait(2000)
        if isLoggedIn then
            irpCore.TriggerServerCallback('irp-housing:server:getOwnedHouses', function(ownedHouses)
...
RegisterNetEvent('irp-housing:client:SetClosestHouse')
AddEventHandler('irp-housing:client:SetClosestHouse', function()
    SetClosestHouse()
end)
...
RegisterNUICallback('buy', function()
    openContract(false)
    disableViewCam()
    TriggerServerEvent('irp-housing:server:buyHouse', closesthouse)
end)
...
RegisterNetEvent('irp-housing:client:viewHouse')
AddEventHandler('irp-housing:client:viewHouse', function(houseprice, brokerfee, bankfee, taxes, firstname, lastname)
...
function SetClosestHouse()
    ...
    if not inside then
        ...
        if closesthouse ~= nil then 
            irpCore.TriggerServerCallback('irp-housing:server:hasKey', function(result)
                hasKey = result
            end, closesthouse)
        
            irpCore.TriggerServerCallback('irp-housing:server:isOwned', function(result)
                isOwned = result
            end, closesthouse)
        end
    end
...
function setHouseLocations()
    if closesthouse ~= nil then
        irpCore.TriggerServerCallback('irp-housing:server:getHouseLocations', function(result)
            if result ~= nil then
...
RegisterNetEvent('irp-housing:client:setLocation')
AddEventHandler('irp-housing:client:setLocation', function(data)
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
    ...
    if inside then
        if hasKey then
            if data == 'stash' then
                TriggerServerEvent('irp-housing:server:setLocation', coords, closesthouse, 1)
            elseif data == 'outift' then
                TriggerServerEvent('irp-housing:server:setLocation', coords, closesthouse, 2)
            elseif data == 'logout' then
                TriggerServerEvent('irp-housing:server:setLocation', coords, closesthouse, 3)
			elseif data == 'stash2' then
                TriggerServerEvent('irp-housing:server:setLocation', coords, closesthouse, 4)
            end
        else
...
RegisterNetEvent('irp-housing:client:refreshLocations')
AddEventHandler('irp-housing:client:refreshLocations', function(house, location, type)
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)

    if closesthouse == house then
        if inside then
            TriggerEvent('irp-housing:getid', closesthouse, house)
            if type == 1 then
