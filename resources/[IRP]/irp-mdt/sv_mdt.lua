    ...
    if xPlayer.job.name == 'police' then
    	MySQL.Async.fetchAll("SELECT * FROM (SELECT * FROM `mdt_reports` ORDER BY `id` DESC LIMIT 3) sub ORDER BY `id` DESC", {}, function(reports)
    		...
    		MySQL.Async.fetchAll("SELECT * FROM (SELECT * FROM `mdt_warrants` ORDER BY `id` DESC LIMIT 3) sub ORDER BY `id` DESC", {}, function(warrants)
    			...
    			TriggerClientEvent('irp-mdt:toggleVisibilty', usource, reports, warrants, officer, xPlayer.job.name)
    		end)
    	end)
    end
end)
...
AddEventHandler("mdt:getOffensesAndOfficer", function()
	local usource = source
	local charges = {}
	MySQL.Async.fetchAll('SELECT * FROM mdt_finetypes', {
	}, function(fines)
		for j = 1, #fines do
...
RegisterServerEvent("irp-mdt:performOffenderSearch")
AddEventHandler("irp-mdt:performOffenderSearch", function(query)
	local usource = source
	local matches = {}
	MySQL.Async.fetchAll("SELECT * FROM `characters` WHERE LOWER(`firstname`) LIKE @query OR LOWER(`lastname`) LIKE @query OR CONCAT(LOWER(`firstname`), ' ', LOWER(`lastname`)) LIKE @query", {
		...
		TriggerClientEvent("irp-mdt:returnOffenderSearchResults", usource, matches)
	end)
end)

RegisterServerEvent("irp-mdt:getOffenderDetails")
AddEventHandler("irp-mdt:getOffenderDetails", function(offender)
	local usource = source
	...
	TriggerClientEvent("irp-mdt:returnOffenderDetails", usource, offender)
end)

RegisterServerEvent("irp-mdt:getOffenderDetailsById")
AddEventHandler("irp-mdt:getOffenderDetailsById", function(char_id)
	local usource = source
	...
	if not offender then
		TriggerClientEvent("irp-mdt:closeModal", usource)
		TriggerClientEvent("irp-mdt:sendNotification", usource, "This person no longer exists.")
		return
	end
...
RegisterServerEvent("irp-mdt:saveOffenderChanges")
AddEventHandler("irp-mdt:saveOffenderChanges", function(id, changes, identifier)
	local usource = source
	MySQL.Async.fetchAll('SELECT * FROM `user_mdt` WHERE `char_id` = @id', {
		...
		TriggerClientEvent("irp-mdt:sendNotification", usource, "Offender changes have been saved.")
	end)
end)

RegisterServerEvent("irp-mdt:saveReportChanges")
AddEventHandler("irp-mdt:saveReportChanges", function(data)
	MySQL.Async.execute('UPDATE `mdt_reports` SET `title` = @title, `incident` = @incident WHERE `id` = @id', {
		['@id'] = data.id,
		['@title'] = data.title,
		['@incident'] = data.incident
	})
	TriggerClientEvent("irp-mdt:sendNotification", source, "Report changes have been saved.")
end)

RegisterServerEvent("irp-mdt:deleteReport")
AddEventHandler("irp-mdt:deleteReport", function(id)
	MySQL.Async.execute('DELETE FROM `mdt_reports` WHERE `id` = @id', {
		['@id']  = id
	})
	TriggerClientEvent("irp-mdt:sendNotification", source, "Report has been successfully deleted.")
end)

RegisterServerEvent("irp-mdt:submitNewReport")
AddEventHandler("irp-mdt:submitNewReport", function(data)
	local usource = source
	...
	MySQL.Async.insert('INSERT INTO `mdt_reports` (`char_id`, `title`, `incident`, `charges`, `author`, `name`, `date`) VALUES (@id, @title, @incident, @charges, @author, @name, @date)', {
		...
		TriggerEvent("irp-mdt:getReportDetailsById", id, usource)
		TriggerClientEvent("irp-mdt:sendNotification", usource, "A new report has been submitted.")
	end)
...
RegisterServerEvent("irp-mdt:performReportSearch")
AddEventHandler("irp-mdt:performReportSearch", function(query)
	local usource = source
	local matches = {}
	MySQL.Async.fetchAll("SELECT * FROM `mdt_reports` WHERE `id` LIKE @query OR LOWER(`title`) LIKE @query OR LOWER(`name`) LIKE @query OR LOWER(`author`) LIKE @query or LOWER(`charges`) LIKE @query", {
		...
		TriggerClientEvent("irp-mdt:returnReportSearchResults", usource, matches)
	end)
end)

RegisterServerEvent("irp-mdt:performVehicleSearch")
AddEventHandler("irp-mdt:performVehicleSearch", function(query)
	local usource = source
	local matches = {}
	MySQL.Async.fetchAll("SELECT * FROM `owned_vehicles` WHERE LOWER(`plate`) LIKE @query", {
		...
		TriggerClientEvent("irp-mdt:returnVehicleSearchResults", usource, matches)
	end)
end)

RegisterServerEvent("irp-mdt:performVehicleSearchInFront")
AddEventHandler("irp-mdt:performVehicleSearchInFront", function(query)
	local usource = source
	local xPlayer = irpCore.GetPlayerFromId(usource)
    if xPlayer.job.name == 'police' then
    	MySQL.Async.fetchAll("SELECT * FROM (SELECT * FROM `mdt_reports` ORDER BY `id` DESC LIMIT 3) sub ORDER BY `id` DESC", {}, function(reports)
    		for r = 1, #reports do
    			reports[r].charges = json.decode(reports[r].charges)
    		...
    		MySQL.Async.fetchAll("SELECT * FROM (SELECT * FROM `mdt_warrants` ORDER BY `id` DESC LIMIT 3) sub ORDER BY `id` DESC", {}, function(warrants)
    			...
    			MySQL.Async.fetchAll("SELECT * FROM `owned_vehicles` WHERE `plate` = @query", {
					...
					local officer = GetCharacterName(usource)
    				TriggerClientEvent('irp-mdt:toggleVisibilty', usource, reports, warrants, officer, xPlayer.job.name)
					TriggerClientEvent("irp-mdt:returnVehicleSearchInFront", usource, result, query)
				end)
    		end)
    	end)
	end
...
RegisterServerEvent("irp-mdt:getVehicle")
AddEventHandler("irp-mdt:getVehicle", function(vehicle)
	local usource = source
	...
	TriggerClientEvent("irp-mdt:returnVehicleDetails", usource, vehicle)
end)

RegisterServerEvent("irp-mdt:getWarrants")
AddEventHandler("irp-mdt:getWarrants", function()
	local usource = source
	MySQL.Async.fetchAll("SELECT * FROM `mdt_warrants`", {}, function(warrants)
		...
		TriggerClientEvent("irp-mdt:returnWarrants", usource, warrants)
	end)
end)

RegisterServerEvent("irp-mdt:submitNewWarrant")
AddEventHandler("irp-mdt:submitNewWarrant", function(data)
	local usource = source
	...
	MySQL.Async.insert('INSERT INTO `mdt_warrants` (`name`, `char_id`, `report_id`, `report_title`, `charges`, `date`, `expire`, `notes`, `author`) VALUES (@name, @char_id, @report_id, @report_title, @charges, @date, @expire, @notes, @author)', {
		...
		TriggerClientEvent("irp-mdt:completedWarrantAction", usource)
		TriggerClientEvent("irp-mdt:sendNotification", usource, "A new warrant has been created.")
	end)
end)

RegisterServerEvent("irp-mdt:deleteWarrant")
AddEventHandler("irp-mdt:deleteWarrant", function(id)
	local usource = source
	...
	TriggerClientEvent("irp-mdt:sendNotification", usource, "Warrant has been successfully deleted.")
end)

RegisterServerEvent("irp-mdt:getReportDetailsById")
AddEventHandler("irp-mdt:getReportDetailsById", function(query, _source)
	...
	MySQL.Async.fetchAll("SELECT * FROM `mdt_reports` WHERE `id` = @query", {
		...
		if result and result[1] then
			...
			TriggerClientEvent("irp-mdt:returnReportDetails", usource, result[1])
		else
			TriggerClientEvent("irp-mdt:closeModal", usource)
			TriggerClientEvent("irp-mdt:sendNotification", usource, "This report cannot be found.")
		end
	end)
end)

RegisterServerEvent("irp-mdt:newCall")
AddEventHandler("irp-mdt:newCall", function(details, caller, coords)
	...
	for i= 1, #xPlayers do
		...
		local xPlayer = irpCore.GetPlayerFromId(source)
		if xPlayer.job.name == 'police' then
			TriggerClientEvent("irp-mdt:newCall", source, details, caller, coords, call_index)
...
RegisterServerEvent("irp-mdt:attachToCall")
AddEventHandler("irp-mdt:attachToCall", function(index)
	local usource = source
	...
	for i= 1, #xPlayers do
		...
		local xPlayer = irpCore.GetPlayerFromId(source)
		if xPlayer.job.name == 'police' then
			TriggerClientEvent("irp-mdt:newCallAttach", source, index, charname)
		end
	end
	TriggerClientEvent("irp-mdt:sendNotification", usource, "You have attached to this call.")
end)

RegisterServerEvent("irp-mdt:detachFromCall")
AddEventHandler("irp-mdt:detachFromCall", function(index)
	local usource = source
	local charname = GetCharacterName(usource)
	local xPlayers = irpCore.GetPlayers()
	for i= 1, #xPlayers do
		...
		local xPlayer = irpCore.GetPlayerFromId(source)
		if xPlayer.job.name == 'police' then
			TriggerClientEvent("irp-mdt:newCallDetach", source, index, charname)
		end
	end
	TriggerClientEvent("irp-mdt:sendNotification", usource, "You have detached from this call.")
end)

RegisterServerEvent("irp-mdt:editCall")
AddEventHandler("irp-mdt:editCall", function(index, details)
	...
	for i= 1, #xPlayers do
		...
		local xPlayer = irpCore.GetPlayerFromId(source)
		if xPlayer.job.name == 'police' then
			TriggerClientEvent("irp-mdt:editCall", source, index, details)
		end
	end
	TriggerClientEvent("irp-mdt:sendNotification", usource, "You have edited this call.")
end)

RegisterServerEvent("irp-mdt:deleteCall")
AddEventHandler("irp-mdt:deleteCall", function(index)
	...
	for i= 1, #xPlayers do
		...
		local xPlayer = irpCore.GetPlayerFromId(source)
		if xPlayer.job.name == 'police' then
			TriggerClientEvent("irp-mdt:deleteCall", source, index)
		end
	end
	TriggerClientEvent("irp-mdt:sendNotification", usource, "You have deleted this call.")
end)

RegisterServerEvent("irp-mdt:saveVehicleChanges")
AddEventHandler("irp-mdt:saveVehicleChanges", function(data)
	...
	MySQL.Async.fetchAll('SELECT * FROM `vehicle_mdt` WHERE `plate` = @plate', {
		...
		TriggerClientEvent("irp-mdt:sendNotification", usource, "Vehicle changes have been saved.")
	end)
end)
