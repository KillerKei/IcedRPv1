...
RegisterServerEvent('irp-identity:setIdentity')
AddEventHandler('irp-identity:setIdentity', function(data, myIdentifiers)
	setIdentity(myIdentifiers.steamid, data, function(callback)
		if callback then
			TriggerClientEvent('irp-identity:identityCheck', myIdentifiers.playerid, true)
		else
...
AddEventHandler('es:playerLoaded', function(source)
	...
	TriggerClientEvent('irp-identity:saveID', source, myID)
	getIdentity(source, function(data)
		if data.firstname == '' then
			TriggerClientEvent('irp-identity:identityCheck', source, false)
			TriggerClientEvent('irp-identity:showRegisterIdentity', source)
		else
			TriggerClientEvent('irp-identity:identityCheck', source, true)
		end
	end)
end)
...
AddEventHandler('onResourceStart', function(resource)
	if resource == GetCurrentResourceName() then
		...
		local xPlayers, xPlayer = irpCore.GetPlayers()
		for i=1, #xPlayers, 1 do
			xPlayer = irpCore.GetPlayerFromId(xPlayers[i])
			...
			TriggerClientEvent('irp-identity:saveID', xPlayer.source, myID)

			getIdentity(xPlayer.source, function(data)
				if data.firstname == '' then
					TriggerClientEvent('irp-identity:identityCheck', xPlayer.source, false)
					TriggerClientEvent('irp-identity:showRegisterIdentity', xPlayer.source)
				else
					TriggerClientEvent('irp-identity:identityCheck', xPlayer.source, true)
				end
			end)
		end
	end
