...
RegisterNetEvent('irp-identity:showRegisterIdentity')
AddEventHandler('irp-identity:showRegisterIdentity', function()
	if not isDead then
		EnableGui(true)
	end
end)
...
RegisterNetEvent('irp-identity:saveID')
AddEventHandler('irp-identity:saveID', function(data)
	myIdentifiers = data
end)
...
RegisterNUICallback('register', function(data, cb)
    ...
    if reason == "" then
		...
		DoScreenFadeIn(1000)
		Wait(1000)
		TriggerServerEvent('irp-identity:setIdentity', data, myIdentifiers)
        EnableGui(false)
		TriggerEvent('irp-clothing:defaultReset', myIdentifiers.id)
		Wait(10000)
