...
RegisterNUICallback('escape', function(data, cb)
    ...
    TriggerEvent("irp-notepad:CloseNotepad")
end)
...
RegisterNetEvent("irp-notepad:OpenNotepadGui")
AddEventHandler("irp-notepad:OpenNotepadGui", function()
    if not isUiOpen then
        openGui()
    end
end)

RegisterNetEvent("irp-notepad:CloseNotepad")
AddEventHandler("irp-notepad:CloseNotepad", function()
    SendNUIMessage({
        action = 'closeNotepad'
    })
    ...
    TaskPlayAnim( player, ad, "exit", 0.5, 0.5, -1, 49, 0, 0, 0, 0 )
...
RegisterNetEvent('irp-notepad:note')
AddEventHandler('irp-notepad:note', function()
    ...
    if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
        ...
        if ( IsEntityPlayingAnim( player, ad, "base", 3 ) ) then 
            ...
            AttachEntityToEntity(prop, player, GetPedBoneIndex(player, 18905), 0.1, 0.02, 0.05, 10.0, 0.0, 0.0, true, true, false, true, 1, true) -- irp-notepadpad
            ...
            TaskPlayAnim( player, ad, "base", 2.0, 1.0, -1, 49, 0, 0, 0, 0 )
        end     
    end
end)
...
RegisterNetEvent('irp-notepad:updateNotes')
AddEventHandler('irp-notepad:updateNotes', function(serverNotesPassed)
