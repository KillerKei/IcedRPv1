...
function HasStolenGoods()
	if OxyRun then
		...
		for i = 84, 97 do
			local itemcount = exports["irp-inventory"]:hasEnoughOfItem(stolenGoodsTable[i],1,false)
			if itemcount then
...
function DoDropOff(requestMoney)
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
		...
		if HasStolenGoods() then
			...
			if OxyRun then
				...
				if exports["irp-inventory"]:hasEnoughOfItem("inkedmoneybag",1,false) then     
					-- RepPlus()
					TriggerEvent("inventory:removeItem","inkedmoneybag", 1)   
					cashPayment = cashPayment + 85000  

		        elseif exports["irp-inventory"]:hasEnoughOfItem("rollcash",30,false) then     
		        ...
		        elseif exports["irp-inventory"]:hasEnoughOfItem("markedbills",20,false) then     
		            TriggerEvent("inventory:removeItem","markedbills", 20)   
		            cashPayment = cashPayment + markedbillsprice            
		            TriggerEvent("DoLongHudText","Thanks for the extra sauce!")
				elseif exports["irp-inventory"]:hasEnoughOfItem("band",20,false) then     
	...
	if success then
		...
		TriggerServerEvent('delmission:completed', cashPayment)
    ...
    while timer > 0 do
        ...
        if not searching then
            ...
            if IsControlJustReleased(1,38) and #(pos - GetEntityCoords(PlayerPedId())) < 3.0 then
                ...
                local finished = exports["irp-taskbar"]:taskBar(15000,"Searching Thug")
...
function CompleteScrapping(vehicle, originalVehicleLocation)
	...
	local finished = exports["irp-taskbar"]:taskBar(20000,"Scrapping Car")
...
AddEventHandler('goldtrade', function()
	if exports["irp-inventory"]:hasEnoughOfItem("goldbar",70,true) then
		...
		TriggerServerEvent('delmission:completed', 35000)
	end
end)
