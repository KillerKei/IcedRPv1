...
----markers
AddEventHandler('irp-duty:hasEnteredMarker', function (zone)
...
AddEventHandler('irp-duty:hasExitedMarker', function (zone)
  CurrentAction = nil
end)
      ...
      if IsControlPressed(0, Keys['E']) then
        if CurrentAction == 'ambulance_duty' then
          ...
          if PlayerData.job.name == 'ambulance' then
            ...
            exports["mumble-voip"]:removePlayerFromRadio(GetPlayerServerId(PlayerId()))
            Wait(1000)
          else
            TriggerEvent("DoLongHudText", "On-Duty 10-41", 1)
            exports["mumble-voip"]:addPlayerToRadio(1.0, GetPlayerServerId(PlayerId()))
            Wait(1000)
          end
        else
  ...
  while true do
    ...
    for k,v in pairs(Config.Zones) do
      if(GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Config.DrawDistance) then
      	irpCore.Game.Utils.DrawText3D(vector3(v.Pos.x, v.Pos.y, v.Pos.z), "Press [E] Go On/Off Duty", 0.4)
      end
    end
  end
end)
...
Citizen.CreateThread(function ()
  while true do
    ...
    if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
      ...
      TriggerEvent('irp-duty:hasEnteredMarker', currentZone)
    end

    if not isInMarker and HasAlreadyEnteredMarker then
      HasAlreadyEnteredMarker = false
      TriggerEvent('irp-duty:hasExitedMarker', LastZone)
    end
  end
end)