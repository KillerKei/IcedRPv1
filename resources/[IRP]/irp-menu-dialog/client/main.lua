Citizen.CreateThread(function()
	...
	local openMenu = function(namespace, name, data)
		for i=1, #Timeouts, 1 do
			irpCore.ClearTimeout(Timeouts[i])
		end
		...
		local timeoutId = irpCore.SetTimeout(200, function()
	...
	irpCore.UI.Menu.RegisterType(MenuType, openMenu, closeMenu)

	RegisterNUICallback('menu_submit', function(data, cb)
		local menu = irpCore.UI.Menu.GetOpened(MenuType, data._namespace, data._name)
		local post = true
		...
		if menu.submit ~= nil then
			...
			if tonumber(data.value) ~= nil then
				...
				-- Round float values
				data.value = irpCore.Math.Round(tonumber(data.value))
