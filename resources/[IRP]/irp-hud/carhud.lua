        ...
        if vehModel ~= nil and vehModel ~= 0 then
            if DoesEntityExist(currVeh) then
                if IsThisModelABoat(vehModel) or IsThisModelAJetski(vehModel) or IsThisModelAnAmphibiousCar(vehModel) or IsThisModelAnAmphibiousQuadbike(vehModel) then
                	local finished = exports["grp-taskbar"]:taskBar(2000,"Toggling Anchor")
...
AddEventHandler("hud:voice:talking", function (talking)
	SendNUIMessage({type = "talkingStatus", is_talking = talking})
end)
...
AddEventHandler('grp-base:playerSessionStarted', function()
