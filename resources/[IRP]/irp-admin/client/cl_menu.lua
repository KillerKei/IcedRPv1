...
function irp.Admin.Menu.DrawTargets(self, cmd, cb)
    ...
    Citizen.CreateThread(function()
        while WarMenu.IsMenuOpened("targetmenu") do
            Citizen.Wait(0)
            for k,v in spairs(irp._Admin.Players, function(t, a, b) return t[a].src < t[b].src end) do
                if WarMenu.MenuButton("[" .. v.src .. "] " .. v.name, "command") then self:DrawCommand(cmd) cb(v) irp.Admin:GetCommandData(cmd).drawcommand() return end
            end
...
Citizen.CreateThread(function()
    ...
    local function DrawPlayers()        
        for k,v in spairs(irp._Admin.Players, function(t, a, b) return t[a].src < t[b].src end) do
            if WarMenu.Button("[" .. v.src .. "] " .. v.name .. (v.sessioned and " - SESSIONED?" or ""), v.status, v.sessioned and {r = 255, g = 0, b = 0, a = 255} or nil) then irp._Admin.Menu.PlayerOptions = v viewingIp = false viewingLicense = false WarMenu.OpenMenu("aplayeropts") end
        end
    ...
    local function DrawDiscPlayers()
        for k,v in spairs(irp._Admin.DiscPlayers, function(t, a, b) return t[a].src < t[b].src end) do
            if WarMenu.MenuButton("[" .. v.src .. "] " .. v.name, "aplayeropts") then irp._Admin.Menu.PlayerOptions = v viewingIp = false end
        end
    ...
    local function DrawPlayerOptions()
        ...
        if options then
            ...
            if irp.Admin:IsSuperAdmin() then
    ...
    local function DrawDev()
        local userRank = irp.Admin:GetPlayerRank()
        if WarMenu.Button("Dev mode: " .. (irp._Admin.Menu.DevMode and "Disable" or "Enable")) and userRank == "superadmin" then 
            irp._Admin.Menu.DevMode = not irp._Admin.Menu.DevMode 
            TriggerEvent("irp-admin:currentDevmode",irp._Admin.Menu.DevMode)
        end
        if WarMenu.Button("Dev debug: " .. (irp._Admin.Menu.DevDebug and "Disable" or "Enable")) and userRank == "superadmin" then 
            irp._Admin.Menu.DevDebug = not irp._Admin.Menu.DevDebug 
            TriggerEvent("irp-admin:currentDebug",irp._Admin.Menu.DevDebug)
            TriggerEvent("hud:enabledebug", nil,irp._Admin.Menu.DevDebug)
        end
        if WarMenu.MenuButton("Back", "amenu") then return end
    end
...
RegisterNetEvent("irp-admin:openMenu")
AddEventHandler("irp-admin:openMenu", function()
    WarMenu.OpenMenu("amenu")
end)

RegisterCommand('menu', function()
    if group ~= "user" then
        TriggerEvent('irp-admin:openMenu')
    else
