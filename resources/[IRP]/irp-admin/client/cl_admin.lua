...
function irp.Admin.CheckForSessions(self)
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            local players = irp._Admin.Players

            for k,v in pairs(players) do
                local src = v.src
                local playerId = GetPlayerFromServerId(src)
                if not src then
                    irp._Admin.Players[src].sessioned = true
                else
                    if not NetworkIsPlayerActive(playerId) or not NetworkIsPlayerConnected(playerId) then irp._Admin.Players[src].sessioned = true end
                end
            end
        end
    end)
...
stop = true
function irp.Admin.BeginAFK(self, stop)
    if stop then
        afk.isAfk = false
        afk.msgAFK = false
        afk.stringToType = ""
    ...
    afk.event = AddEventHandler("irp-admin:afkStringCheck", function(text)
        if string.lower(text) == afk.stringToType then irp.Admin:BeginAFK(true) return end
    end)
    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            if curTime - beginTime >= 300000 then TriggerServerEvent("irp-admin:Disconnect", "AFK Kick") return end
        end
    end)
end

function irp.Admin.AFKCheck(self)
...
RegisterNetEvent("irp-admin:afkStringCheck")

RegisterNetEvent("irp-admin:setStatus")
AddEventHandler("irp-admin:setStatus", function(src, status)
    local player = irp._Admin.Players[src]
    if not player then return else irp._Admin.Players[src].status = status end
end)

RegisterNetEvent("irp-admin:sendPlayerInfo")
AddEventHandler("irp-admin:sendPlayerInfo", function(data, discData)
    irp._Admin.Players = data
    irp._Admin.DiscPlayers = discData
end)

RegisterNetEvent("irp-admin:RemovePlayer")
AddEventHandler("irp-admin:RemovePlayer", function(data)
    irp._Admin.DiscPlayers[data.src] = data
    irp._Admin.Players[data.src] = nil
end)

RegisterNetEvent("irp-admin:RemoveRecent")
AddEventHandler("irp-admin:RemoveRecent", function(data)
    if data ~= nil then
        irp._Admin.DiscPlayers[data.src] = nil
    end
end)

RegisterNetEvent("irp-admin:AddPlayer")
AddEventHandler("irp-admin:AddPlayer", function(player)
    irp._Admin.Players[player.src] = player
end)

RegisterNetEvent('irp-admin:teleportUser')
AddEventHandler('irp-admin:teleportUser', function(x, y, z)
    SetEntityCoords(PlayerPedId(), x, y, z)
    TriggerEvent('DoLongHudText', 'You were summoned')
end)

RegisterNetEvent("irp-admin:AddAllPlayers")
AddEventHandler("irp-admin:AddAllPlayers", function(data)
    irp._Admin.Players[data.src] = data
end)

function irp.Admin.RunCommand(self, args)
    if not args or not args.command then return end
    TriggerServerEvent("irp-admin:runCommand", args)
end
...
RegisterNetEvent("irp-admin:RunClCommand")
AddEventHandler("irp-admin:RunClCommand", function(cmd, args)
    irp.Admin:RunClCommand(cmd, args)
end)

RegisterNetEvent("irp-admin:updateData")
AddEventHandler("irp-admin:updateData", function(src, type, data)
...
RegisterNetEvent("irp-admin:noLongerAdmin")
AddEventHandler("irp-admin:noLongerAdmin", function()
    irp._Admin.Players = {}
...
RegisterNetEvent("irp-admin:bringPlayer")
AddEventHandler("irp-admin:bringPlayer", function(targPos)
    local ped = PlayerPedId()
...
RegisterNetEvent("irp-admin:bring")
AddEventHandler("irp-admin:bring", function(target)
    ...
    TriggerServerEvent("irp-admin:bringPlayerServer",pos,target)   
end)



 
RegisterNetEvent("irp-admin:runSpawnCommand")
AddEventHandler("irp-admin:runSpawnCommand", function(model, livery)
    Citizen.CreateThread(function()
...
RegisterNetEvent("irp-admin:ReviveInDistance")
AddEventHandler("irp-admin:ReviveInDistance", function()
    local playerList = {}
