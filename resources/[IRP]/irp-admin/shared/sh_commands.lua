...
function cmd.RunCommand(caller, args)
    ...
    --irp.Admin:SetRank(caller.target, caller.rank)
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter a ban length", "Length: " .. (cmd.vars.time and cmd.vars.time or "No Time Given")) then
        irp.Admin.Menu:DrawTextInput(cmd.vars.time and cmd.vars.time or "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end

                local timeTable, timeSum, addedTime = irp.Admin:GetBanTimeFromString(result)
    ...
    if cmd.vars.target and cmd.vars.time then if WarMenu.Button("Ban " .. cmd.vars.target.name) then irp.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, reason = cmd.vars.reason, time = cmd.vars.time}) cmd.vars.target = nil cmd.vars.reason = nil cmd.vars.time = nil end end
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter a Steam ID", "Entered: " .. (cmd.vars.steamid and cmd.vars.steamid or "None")) then 
        irp.Admin.Menu:ShowTextEntry("Enter a Steam ID", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.steamid then if WarMenu.Button("Unban " .. cmd.vars.steamid) then irp.Admin:GetCommandData(cmd.command).runcommand({steamid = cmd.vars.steamid}) cmd.vars.steamid = nil end end
end
...
function cmd.RunCommand(caller, args)
    ...
    TriggerServerEvent('irp-admin:Cloak', src, caller.toggle)
end
...
function cmd.Init()
    ...
    if IsDuplicityVersion() then 
        AddEventHandler("playerDropped", function()
            ...
            if cmd.vars.cloaked[src] then
                TriggerServerEvent('irp-admin:Cloak', src, false)
    ...
    RegisterNetEvent("irp-admin:CloakRemote")
    AddEventHandler("irp-admin:CloakRemote", function()
    ...
    RegisterNetEvent("irp-admin:Cloak")
    AddEventHandler("irp-admin:Cloak", function(player, toggle)
...
function cmd.RunCommand(caller, args)
	...
	TriggerEvent('irp-hospital:client:RemoveBleed', caller.target.src) 
	TriggerEvent('irp-hospital:client:ResetLimbs', caller.target.src)
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Revive in Distance") then 
        TriggerEvent("irp-admin:ReviveInDistance")
    end
...
function cmd.RunCommand(caller, args)
    if not caller.message then return end
    local src = source
    TriggerEvent("irp-admin:runSpawnCommand", caller.message)
    local playerName = GetPlayerName(PlayerId())
...
function cmd.DrawCommand()
    ...
    if cmd.vars.message then 
        if WarMenu.Button("Spawn model: " .. cmd.vars.message) then 
            irp.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message}) 
        end 
    end
end
...
cmd = {
    ...
    category = "Utility",
    usage = "smoney <amount>",
    description = "Sets Money (NOT ADD)",
    ranks = {"superadmin"},
    vars = {}
}
...
function cmd.RunCommand(caller, args)
    if not caller.message then return end
    local src = source
    local moneylol = caller.message
    TriggerServerEvent("AdminMenu:MoneyLol", moneylol)
    local playerName = GetPlayerName(PlayerId())
    TriggerServerEvent('adminmenu', playerName .. " has spawned money:- ".. caller.message)
end

function cmd.DrawCommand()
    cmd.vars.message = cmd.vars.message ~= nil and cmd.vars.message or nil

    if WarMenu.Button("Enter amount of money", "Money: " .. (cmd.vars.message and cmd.vars.message or "No Money")) then
        irp.Admin.Menu:ShowTextEntry("Enter amount", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.message then 
        if WarMenu.Button("Spawn money: " .. cmd.vars.message) then 
            irp.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message}) 
        end 
    end
end
...
function cmd.RunCommand(caller, args)
    ...
    irp.Admin:Log(log, caller)
    irp.Admin:Bring(caller,args.target.source)
end
...
RegisterNetEvent('irp-admin:toggleRave')
AddEventHandler('irp-admin:toggleRave', function(toggle)
    toggles = toggle
...
function cmd.DrawCommand()
    cmd.vars.toggle = cmd.vars.toggle ~= nil and cmd.vars.toggle or nil
    if WarMenu.Button("Player Blips: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle irp.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle}) end
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Fix Current Vehicle") then 
        ...
        irp.Admin:GetCommandData(cmd.command).runcommand()
    end
end
