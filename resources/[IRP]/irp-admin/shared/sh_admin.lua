...
function irp.Admin.SetStatus(self, status, src)
    ...
    if IsDuplicityVersion() then
        ...
        for k,v in pairs(irp._Admin.CurAdmins) do
            TriggerClientEvent("irp-admin:setStatus", k, src, status)
        end

        return
    end

    TriggerServerEvent("irp-admin:setStatus", status)
end

AddEventHandler("np-base:exportsReady", function()
    exports["np-base"]:addModule("Admin", irp.Admin)
    exports["np-base"]:addModule("_Admin", irp._Admin)
end)