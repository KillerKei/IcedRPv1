irp = irp or {}
irp.Doors = irp.Doors or {}
irp.DoorCoords = irp.DoorCoords or {}
irp.offSet = irp.offSet or {}
...
function irp.alterState(alterNum)

    if irp.DoorCoords[alterNum]["lock"] == 0 then 
        irp.DoorCoords[alterNum]["lock"] = 1 
    else 
        irp.DoorCoords[alterNum]["lock"] = 0 
    end
    TriggerClientEvent("irp:Door:alterState",-1,alterNum,irp.DoorCoords[alterNum]["lock"])

end

RegisterNetEvent( 'irp:Door:alterState' )
AddEventHandler( 'irp:Door:alterState', function(alterNum,num)
	irp.DoorCoords[alterNum]["lock"] = num
end)

RegisterNetEvent("irp-doors:alterlockstateclient")
AddEventHandler("irp-doors:alterlockstateclient", function(doorCoords)
    irp.DoorCoords = doorCoords
end)
