...
RegisterCommand('impound', function(source)
	if irpCore.GetPlayerData().job.name == 'police' or irpCore.GetPlayerData().job.name == 'mechanic' or irpCore.GetPlayerData().job.name == 'ambulance' then
...
RegisterCommand("extra", function(source, args, rawCommand)
  ...
  if PlayerData.job.name == 'police' then
	...
	local veh = GetVehiclePedIsIn(ped, false)
    	...
    	if toggle == "true" then
			toggle = 0
			exports["irp-taskbar"]:taskBar(800, "Adding Extra")
    		end
		if veh ~= nil and veh ~= 0 and veh ~= 1 then
			exports["irp-taskbar"]:taskBar(800, "Removing Extra")
...
RegisterCommand('tint', function(source, args, raw)
	if irpCore.GetPlayerData().job.name == 'police' or irpCore.GetPlayerData().job.name == 'ambulance' then
		...
		exports["irp-taskbar"]:taskBar(800, "Tinting Vehicle")
...
RegisterCommand('livery', function(source, args, raw)
    ...
    if tonumber(args[1]) ~= nil and irpCore.GetPlayerData().job.name == 'police' or irpCore.GetPlayerData().job.name == 'ambulance' and GetVehicleLiveryCount(vehicle) - 1 >= tonumber(args[1]) then
		exports["irp-taskbar"]:taskBar(900, "Changing Livery")
