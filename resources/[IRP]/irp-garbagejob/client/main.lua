	...
	irpCore.UI.Menu.Open(
		...
		function(data, menu)
			irpCore.Game.SpawnVehicle(data.current.value, Config.Zones.VehicleSpawnPoint.Pos, 270.0, function(vehicle)
				...
				exports["irp-oGasStations"]:SetFuel(vehicle, 100)		
...
function SelectBinandCrew()
	...
	local NewBin, NewBinDistance = irpCore.Game.GetClosestObject(Config.DumpstersAvaialbe)
...
Citizen.CreateThread(function()
	while true do
		...
		if truckdeposit then
			...
			dist = Vdist(plyCoords.x, plyCoords.y, plyCoords.z, trunk.x , trunk.y, trunk.z)
			if dist <= 2.0 then
			irpCore.Game.Utils.DrawText3D(vector3(trunk.x , trunk.y ,trunk.z + 0.50), "[~g~E~s~] Throw bag in truck.", 1.0)
			end
		end
		...
		if trashcollection then
			...
			if dist <= 5.0 then
				if currentbag <= 0 then
					if iscurrentboss then
					irpCore.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "[~g~E~s~] Clean up area", 1.0)		
					else
					irpCore.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "Collection done.. Wait on truck..", 1.0)		
					end
				else
					irpCore.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "[~g~E~s~] Collect bag from Trashbin ["..currentbag.."/"..bagsoftrash.."]", 1.0)
				end
			end
		end
