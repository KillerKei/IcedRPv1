...
RegisterNUICallback('btnTaskGroups', function()
    local jobgrade = irpCore.PlayerData.job.grade
    local rankConverted = irpCore.PlayerData.job.grade_label

    local groupObject = {}
    table.insert(groupObject, {
        namesent = irpCore.PlayerData.job.label,
...
RegisterNUICallback('vehiclePayImpound', function(data)
  ...
  TriggerEvent('DoLongHudText', 'You payed $700 for this transaction with tax.')
end)

function findVehFromPlateAndLocate(plate)


  local gameVehicles = irpCore.Game.GetVehicles()
...
RegisterNUICallback('btnAttempCreateHouse', function(data, cb)
  TriggerEvent("irp-housing:client:createHouses", data.price, data.tier)
  loadedHouse = false
...
function hasPhone()
  ...
  if
    (
    (exports["irp-inventory"]:hasEnoughOfItem("mobilephone",1,false) or 
    exports["irp-inventory"]:hasEnoughOfItem("stoleniphone",1,false) or 
    exports["irp-inventory"]:hasEnoughOfItem("stolens8",1,false) or
    exports["irp-inventory"]:hasEnoughOfItem("stolennokia",1,false) or
    exports["irp-inventory"]:hasEnoughOfItem("stolenpixel3",1,false) or
    exports["irp-inventory"]:hasEnoughOfItem("boomerphone",1,false))
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsDisabledControlJustReleased(1, 199) then
      TriggerEvent("irp:heHasPhone")
    end
  end
end)

RegisterNetEvent('irp:heHasPhone')
AddEventHandler('irp:heHasPhone', function()
    GotPhone()
end)


function GotPhone()
  local dead = exports["irp-ambulancejob"]:GetDeath()
      if not dead then
        openGuiNow()
      else
...
function openGuiNow()
  ...
  if hasPhone() then
    ...
    if irpCore.PlayerData.job.name == "realestateagent" then
     isREAgent = true
    end
...
RegisterNUICallback('newPostSubmit', function(data, cb)
  if irpCore.PlayerData.job.name ~= "taxi" and irpCore.PlayerData.job.name ~= "towtruck" and irpCore.PlayerData.job.name ~= "trucker" then
    TriggerServerEvent('phone:updatePhoneJob', data.advert)
  else
    TriggerServerEvent('phone:updatePhoneJob', irpCore.PlayerData.job.name .. " | " .. data.advert)
...
AddEventHandler('getAccountInfo', function(cash, bank, licences)
    local responseObject = {
        cash = cash,
        bank = bank,
        job = irpCore.PlayerData.job.name .. ', ' .. irpCore.PlayerData.job.grade_label,
...
AddEventHandler('phone:newContact', function(name, number)
  ...
  TriggerEvent('DoLongHudText', 'Contact Saved!', 1)
  TriggerServerEvent('getContacts')
end)
...
RegisterNetEvent("irp-phone:userTwitter")

AddEventHandler("irp-phone:userTwitter", function(user)
  userTwitter = user
 end)
