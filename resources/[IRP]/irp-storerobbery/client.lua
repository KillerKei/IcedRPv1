...
RegisterNetEvent('irp-storerobbery:onPedDeath')
AddEventHandler('irp-storerobbery:onPedDeath', function(store)
    SetEntityHealth(peds[store], 0)
end)

RegisterNetEvent('irp-storerobbery:msgPolice')
AddEventHandler('irp-storerobbery:msgPolice', function(store, robber)
    --TriggerEvent('irp-alerts:storerob')
    TriggerEvent('irp-dispatch:storerobbery')
end)

RegisterNetEvent('irp-storerobbery:removePickup')
AddEventHandler('irp-storerobbery:removePickup', function(bank)
    for i = 1, #objects do 
...
RegisterNetEvent('irp-storerobbery:robberyOver')
AddEventHandler('irp-storerobbery:robberyOver', function()
    robbing = false
end)

RegisterNetEvent('irp-storerobbery:talk')
AddEventHandler('irp-storerobbery:talk', function(store, text, time)
    robbing = false
...
RegisterNetEvent('irp-storerobbery:rob')
AddEventHandler('irp-storerobbery:rob', function(i)
                    ...
                    while true do
                        ...
                        if DoesEntityExist(bag) then
                            if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), GetEntityCoords(bag), true) <= 1.5 then
                                ...
                                TriggerServerEvent('irp-storerobbery:pickUp', i)
                                break
                            end
                        else
                            break
...
RegisterNetEvent('irp-storerobbery:resetStore')
AddEventHandler('irp-storerobbery:resetStore', function(i)
    while not irpCoreLoaded do Wait(0) end
...
Citizen.CreateThread(function()
    ...
    Citizen.CreateThread(function()
        while true do
            for i = 1, #peds do
                if IsPedDeadOrDying(peds[i]) then
                    TriggerServerEvent('irp-storerobbery:pedDead', i)
                end
            end
            Wait(5000)
        end
            ...
            if IsPlayerFreeAiming(PlayerId()) then
                for i = 1, #peds do
                    if HasEntityClearLosToEntityInFront(me, peds[i], 19) and not IsPedDeadOrDying(peds[i]) and GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 5.0 then
                        if not robbing then
                            local canRob = nil
                            irpCore.TriggerServerCallback('irp-storerobbery:canRob', function(cb)
                                canRob = cb
                            end, i)
                            while canRob == nil do
                                Wait(0)
                            ...
                            if canRob == true then
                                ...
                                TriggerServerEvent('irp-storerobbery:notif', i)
                                ...
                                if GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 7.5 then
                                    if not IsPedDeadOrDying(peds[i]) then
                                        TriggerServerEvent('irp-storerobbery:rob', i)
                                        while robbing do Wait(0) if IsPedDeadOrDying(peds[i]) then robbing = false end end
                                    end
                                else
                                    ClearPedTasks(peds[i])
                            ...
                            elseif canRob == 'no_cops' then
                                ...
                                TriggerEvent('irp-storerobbery:talk', i, '~g~*' .. Translation[Config.Locale]['shopkeeper'] .. '* ~w~' .. Translation[Config.Locale]['robbed'], 5)
                                Wait(2500)
                            end
                        end
                    end
