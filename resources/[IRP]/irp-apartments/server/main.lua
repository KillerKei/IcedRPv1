...
irpCore.RegisterServerCallback('apartments:GetAvailableApartments', function(source, cb, apartment)
    local apartments = {}
...
irpCore.RegisterServerCallback('apartments:GetApartmentOffset', function(source, cb, apartmentId)
    local retval = 0
...
irpCore.RegisterServerCallback('apartments:GetApartmentOffsetNewOffset', function(source, cb, apartment)
...
irpCore.RegisterServerCallback('apartments:GetOwnedApartment', function(source, cb, cid)
    if cid ~= nil then
...
irpCore.RegisterServerCallback('apartments:IsOwner', function(source, cb, apartment)
	local src = source
    local Player = irpCore.GetPlayerFromId(src)
    if Player ~= nil then
        MySQL.Async.fetchAll("SELECT * FROM apartments WHERE  identifier = @identifier",{['@identifier'] = Player.identifier}, function(result)
