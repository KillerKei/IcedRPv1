            ...
            if InApartment then
                ...
                if IsOwned then
                    ...
                    if(GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Apartments.Locations[ClosestHouse].coords.enter.x, Apartments.Locations[ClosestHouse].coords.enter.y,Apartments.Locations[ClosestHouse].coords.enter.z, true) < 1.2)then
                        ...
                        if IsControlJustPressed(0, Keys["E"]) then
                            irpCore.TriggerServerCallback('apartments:GetOwnedApartment', function(result)
                                if result ~= nil then
...
function apartal()
	...
	TriggerEvent("irp-uimenu:createMenu", myListMenu, function(data)
...
RegisterNetEvent('irp-apartments:client:LastLocationHouse')
AddEventHandler('irp-apartments:client:LastLocationHouse', function(apartmentType, apartmentId)
...
function EnterApartment(house, apartmentId, new)
    ...
    openHouseAnim()
    Citizen.Wait(250)
    irpCore.TriggerServerCallback('apartments:GetApartmentOffset', function(offset)
        if offset == nil or offset == 0 then
            irpCore.TriggerServerCallback('apartments:GetApartmentOffsetNewOffset', function(newoffset)
                ...
                data = exports['irp-interior']:CreateApartmentFurnished(coords)
                Citizen.Wait(100)
                houseObj = data[1]
                POIOffsets = data[2]
        ...
        if new ~= nil then
            if new then
                TriggerEvent('irp-interior:client:SetNewState', true)
            else
                TriggerEvent('irp-interior:client:SetNewState', false)
            end
        else
            TriggerEvent('irp-interior:client:SetNewState', false)
        end
...
function SetClosestApartment()
    ...
    if current ~= ClosestHouse  and not InApartment then
        ...
        irpCore.TriggerServerCallback('apartments:IsOwner', function(result)
            IsOwned = result
        end, ClosestHouse)
    end
end
...
function OwnerList()
    irpCore.TriggerServerCallback('apartments:GetAvailableApartments', function(apartments)
        ped = GetPlayerPed(-1);
