    ...
    while true do
        ...
        if dis <= 5 then
            ...
            if IsControlJustReleased(0, 38) then
                local finished = exports["irp-taskbar"]:taskBar(2000,"Selling Fish",true,false,playerVeh)
        ...
        if dis3 <= 5 and veh == 0 then
            ...
            if IsControlJustReleased(0, 38) then
                if canSpawn == true then
                    if irpCore.GetPlayerData().money >= 500 then
                    TriggerServerEvent('fish:checkAndTakeDepo')
                    Citizen.Wait(500)
                        canSpawn = false
                        irpCore.Game.SpawnVehicle('marquis', vector3(-3448.48, 971.98, 1.91), 45, function(vehicle) 
                            veh = vehicle
                            SetEntityAsMissionEntity(veh, true, true)
                            TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
                            TriggerServerEvent('garage:addKeys', GetVehicleNumberPlateText(veh))
                        end)
                    elseif irpCore.GetPlayerData().money < 500 then
        ...
        if dis4 <= 5 and veh == 0 then
            ...
            if IsControlJustReleased(0, 38) then
                if canSpawn == true then
                    ...
                    Citizen.Wait(500)
                        canSpawn = false
                        irpCore.Game.SpawnVehicle('suntrap', vector3(1299.69, 4194.82, 30.91), 45, function(vehicle) 
                            veh = vehicle
                            SetEntityAsMissionEntity(veh, true, true)
                            TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
                            TriggerServerEvent('garage:addKeys', GetVehicleNumberPlateText(veh))
        ...
        if dis5 <= 5 and veh == 0 then
            ...
            if IsControlJustReleased(0, 38) then
                if canSpawn == true then
                    ...
                    Citizen.Wait(500)
                        canSpawn = false
                        irpCore.Game.SpawnVehicle('tropic', vector3(3865.89, 4476.66, 1.53), 45, function(vehicle) 
                            veh = vehicle
                            SetEntityAsMissionEntity(veh, true, true)
                            TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
                            TriggerServerEvent('garage:addKeys', GetVehicleNumberPlateText(veh))
...
RegisterNetEvent('irp-fish:lego')
AddEventHandler('irp-fish:lego', function()
...
function StartFish()
    ...
    if IsEntityInWater(boat) and IsPedSwimming(ply) == false and inZone == true then
        if exports["irp-inventory"]:hasEnoughOfItem('fishingrod',1,false) then
            isFishing = true
            cancel = false
            Fish()
        end
...
function Catch()
    if cancel == false then
        ...
        local finished = exports["irp-taskbarskill"]:taskBar(math.random(1000,2000),math.random(15,20))
        if finished == 100 then
            local finished2 = exports["irp-taskbarskill"]:taskBar(3000,math.random(2,5))
            if finished2 == 100 then
                ...
                elseif rdn > 11 then
                    ...
                    TriggerServerEvent('irp-fish:getFish')
                    SetCurrentPedWeapon(ply, `WEAPON_UNARMED`, true)
                    ClearPedTasksImmediately(ply)
                end
...
function SellItems()
    local sellfish = math.random(60, 80)
        if exports["irp-inventory"]:hasEnoughOfItem("fish",2,false) then 
        TriggerEvent("inventory:removeItem", "fish", 2)
        Citizen.Wait(1000)
        TriggerEvent('loopUpdateItems')
        Citizen.Wait(1000)
        TriggerServerEvent('irp-fish:payShit', sellfish)
    else
