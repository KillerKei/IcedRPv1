...
AddEventHandler('dispatch:clNotify', function(pNotificationData)
  local currentJob = irpCore.GetPlayerData().job.name
            ...
            for key, value in pairs(pNotificationData.recipientList) do
                if key == currentJob and value and not disableNotifications then
                    ...
                    elseif currentJob == "news" then
                        if exports["irp-inventory"]:getQuantity("scanner") > 0 then
                            local newsObject = {}
...
Citizen.CreateThread(function()
	while true do
        Citizen.Wait(0)
        if (IsControlJustReleased(0, 178) and (irpCore.GetPlayerData().job.name == 'ambulance' or irpCore.GetPlayerData().job.name == 'police') and not exports["irp-ambulancejob"]:GetDeath()) then
...
AddEventHandler('police:panic', function()
      ...
      recipientList = {
		...
		TriggerEvent('irp-alerts:policepanic')
end)
...
AddEventHandler('police:assistance', function()
      ...
      recipientList = {
		...
		TriggerEvent('irp-alerts:assistanceneeded')
end)
...
AddEventHandler("police:1047", function()
    ...
    recipientList = {
      police = "police", ambulance = "ambulance"
		...
		TriggerEvent('irp-alerts:downguy')
end)
...
function AlertGunShot()
    Citizen.CreateThread(function() 
      ...
      TriggerEvent('irp-alerts:gunshot')
...
function CarCrash()
  ...
  TriggerEvent('irp-alerts:vehiclecrash')
...
function AlertCheckLockpick(object)
  ...
  if not DoesEntityExist(targetVehicle) then
    ...
    TriggerEvent('irp-alerts:stolenveh')
  end
end
...
function AlertpersonRobbed(vehicle)
  ...
  TriggerEvent('irp-alerts:robstore')
  Wait(math.random(5000,15000))
...
function AlertCheckRobbery2()
  ...
  TriggerEvent('irp-alerts:robhouse')
  Wait(math.random(5000,15000))
...
function AlertJewelRob()
  ...
  TriggerEvent('irp-alerts:jewrob')
  Wait(math.random(5000,15000))
...
RegisterNetEvent('irp-dispatch:drugjob')
AddEventHandler("irp-dispatch:drugjob",function()
  AlertDrugJob()
end)

RegisterNetEvent('irp-dispatch:bankrobbery')
AddEventHandler("irp-dispatch:bankrobbery",function()
  AlertFleecaRobbery()
end)

RegisterNetEvent('irp-dispatch:jailbreak')
AddEventHandler("irp-dispatch:jailbreak",function()
  AlertJailBreak()
end)

RegisterNetEvent('irp-dispatch:jewelrobbery')
AddEventHandler("irp-dispatch:jewelrobbery",function()
  AlertJewelRob()
  return
end)

RegisterNetEvent('irp-dispatch:houserobbery')
AddEventHandler("irp-dispatch:houserobbery",function()
  AlertCheckRobbery2()
end)

RegisterNetEvent('irp-dispatch:storerobbery')
AddEventHandler("irp-dispatch:storerobbery",function()
  AlertpersonRobbed(vehicle)
end)

RegisterNetEvent('irp-dispatch:carjacking')
AddEventHandler("irp-dispatch:carjacking",function()
  AlertCheckLockpick(object)
end)

RegisterNetEvent('irp-ambulancejob:downplayer')
AddEventHandler("irp-ambulancejob:downplayer",function()
    AlertDeath()
end)
