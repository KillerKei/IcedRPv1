...
AddEventHandler('playerSpawned', function()
	...
	if FirstSpawn then
		...
		irpCore.TriggerServerCallback('irp-ambulancejob:getDeathStatus', function(isDead)
...
function OnPlayerDeath()
	...
	TriggerServerEvent('irp-ambulancejob:setDeathState', true)
...
function StartDeathTimer()
	...
	if Config.EarlyRespawnFine then
		irpCore.TriggerServerCallback('irp-ambulancejob:checkBalance', function(canPay)
			canPayFine = canPay
		end)
	end

	local earlySpawnTimer = irpCore.Math.Round(Config.EarlyRespawnTimer / 1000)
	local bleedoutTimer = irpCore.Math.Round(Config.BleedoutTimer / 1000)
	...
	Citizen.CreateThread(function()
		...
		while bleedoutTimer > 0 and IsDead do
			...
			elseif Config.EarlyRespawnFine and canPayFine then
				text = text .. _U('respawn_bleedout_fine', irpCore.Math.GroupDigits(Config.EarlyRespawnFineAmount))

				if IsControlPressed(0, Keys['H']) and timeHeld > 60 then
					TriggerServerEvent('irp-ambulancejob:payFine')
					RemoveItemsAfterRPDeath()
...
function RemoveItemsAfterRPDeath()
	TriggerServerEvent('irp-ambulancejob:setDeathState', false)
	...
	Citizen.CreateThread(function()
		...
		irpCore.TriggerServerCallback('irp-ambulancejob:removeItemsAfterRPDeath', function()
			local playerpos = GetEntityCoords( GetPlayerPed(-1) )
				
			irpCore.SetPlayerData('lastPosition', playerpos)
			irpCore.SetPlayerData('loadout', {})
			RespawnPed(PlayerPedId(), playerpos, Config.RespawnPoint.heading)

			TriggerServerEvent('irp:updateLastPosition', playerpos)
...
RegisterNetEvent('irp-ambulancejob:useItem')
AddEventHandler('irp-ambulancejob:useItem', function(itemName)
	irpCore.UI.Menu.CloseAll()
	...
	if itemName == 'medikit' then
		...
		exports["irp-taskbar"]:taskBar(10000, "Healing")
	
			TriggerEvent('irp-ambulancejob:heal', 'big', true)
			TriggerEvent('irp-hospital:client:RemoveBleed')
			TriggerEvent('irp-hospital:client:ResetLimbs')
	...
	elseif itemName == 'bandage' then
		...
		exports["irp-taskbar"]:taskBar(10000, "Healing")
			...
			TriggerEvent('irp-ambulancejob:heal', 'small', true)
			TriggerEvent('irp-hospital:client:RemoveBleed')
			ClearPedTasks(playerPed)
	end
end)
...
function RespawnPed(ped, coords, heading)
	...
	TriggerEvent('irp-hospital:client:RemoveBleed', ped) 
	TriggerEvent('irp-hospital:client:ResetLimbs', ped)
...
RegisterNetEvent('irp-ambulancejob:revive')
AddEventHandler('irp-ambulancejob:revive', function()
	attemptRevive()
end)
