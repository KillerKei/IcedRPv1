...
RegisterNetEvent('irp-ambulancejob:drag')
AddEventHandler('irp-ambulancejob:drag', function(copID)
...
RegisterCommand('escort', function()
	...
	if closestPlayer ~= -1 and closestDistance <= 3.0 then
		TriggerServerEvent('irp-ambulancejob:drag', GetPlayerServerId(closestPlayer))
	else
...
function OpenAmbulanceActionsMenu()
	...
	if Config.EnablePlayerManagement and irpCore.PlayerData.job.grade_name == 'boss' then
		table.insert(elements, {label = _U('boss_actions'), value = 'boss_actions'})
	end

	irpCore.UI.Menu.CloseAll()

	irpCore.UI.Menu.Open('default', GetCurrentResourceName(), 'ambulance_actions', {
		title    = _U('ambulance'),
		align    = 'bottom-right',
		elements = elements
	}, function(data, menu)
		...
		elseif data.current.value == 'boss_actions' then
			TriggerEvent('irp-society:openBossMenu', 'ambulance', function(data, menu)
				menu.close()
			end, {wash = false})
		end
...
function OpenMobileAmbulanceActionsMenu()
	...
	irpCore.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_ambulance_actions', {
		title    = _U('ambulance'),
		align    = 'bottom-right',
		elements = {
			...
			irpCore.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer == -1 or closestDistance > 2.0 then
					...
					if data.current.value == 'revive' then
						...
						irpCore.TriggerServerCallback('irp-ambulancejob:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_a", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_b", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_c", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_d", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_e", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_f", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_g", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_h", 3)then
									...
									for i=1, 15, 1 do
										...
										irpCore.Streaming.RequestAnimDict(lib, function()
									...
									TriggerServerEvent('irp-ambulancejob:removeItem', 'medikit')
									TriggerServerEvent('irp-ambulancejob:revive', GetPlayerServerId(closestPlayer))
						...
						irpCore.TriggerServerCallback('irp-ambulancejob:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if health > 0 then
									...
									TriggerServerEvent('irp-ambulancejob:removeItem', 'bandage')
									TriggerServerEvent('irp-ambulancejob:heal', GetPlayerServerId(closestPlayer), 'small')
						...
						irpCore.TriggerServerCallback('irp-ambulancejob:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if health > 0 then
									...
									TriggerServerEvent('irp-ambulancejob:removeItem', 'medikit')
									TriggerServerEvent('irp-ambulancejob:heal', GetPlayerServerId(closestPlayer), 'big')
					...
					elseif data.current.value == 'put_in_vehicle' then
						TriggerServerEvent('irp-ambulancejob:putInVehicle', GetPlayerServerId(closestPlayer))
					elseif data.current.value == 'pull_out_vehicle' then
						TriggerServerEvent('irp-ambulancejob:pullOutVehicle', GetPlayerServerId(closestPlayer))
					elseif data.current.value == 'drag_player' then
						TriggerServerEvent('irp-ambulancejob:drag', GetPlayerServerId(closestPlayer))
					elseif data.current.value == 'un_drag' then
						TriggerServerEvent('irp-ambulancejob:undrag', GetPlayerServerId(closestPlayer))
					end
				end
			end, function(data, menu)
				menu.close()
...
function FastTravel(coords, heading)
	...
	irpCore.Game.Teleport(playerPed, coords, function()
		DoScreenFadeIn(800)
...
Citizen.CreateThread(function()
	while true do
		...
		if irpCore.PlayerData.job and irpCore.PlayerData.job.name == 'ambulance' then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastHospital ~= currentHospital or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('irp-ambulancejob:hasExitedMarker', LastHospital, LastPart, LastPartNum)
					hasExited = true
				end

				HasAlreadyEnteredMarker, LastHospital, LastPart, LastPartNum = true, currentHospital, currentPart, currentPartNum

				TriggerEvent('irp-ambulancejob:hasEnteredMarker', currentHospital, currentPart, currentPartNum)
...
AddEventHandler('irp-ambulancejob:hasEnteredMarker', function(hospital, part, partNum)
	if irpCore.PlayerData.job and irpCore.PlayerData.job.name == 'ambulance' then
...
AddEventHandler('irp-ambulancejob:hasExitedMarker', function(hospital, part, partNum)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			irpCore.ShowHelpNotification(CurrentActionMsg)
		...
		elseif irpCore.PlayerData.job ~= nil and irpCore.PlayerData.job.name == 'ambulance' and not IsDead then
...
RegisterNetEvent('irp-ambulancejob:putInVehicle')
AddEventHandler('irp-ambulancejob:putInVehicle', function()
	local playerPed = PlayerPedId()
...
RegisterNetEvent('irp-ambulancejob:pullOutVehicle')
AddEventHandler('irp-ambulancejob:pullOutVehicle', function()
	local playerPed = PlayerPedId()
...
RegisterNetEvent('irp-ambulancejob:drag')
AddEventHandler('irp-ambulancejob:drag', function(cop)
	IsDragged = not IsDragged
	CopPed = tonumber(cop)
end)

RegisterNetEvent('irp-ambulancejob:un_drag')
AddEventHandler('irp-ambulancejob:un_drag', function(cop)
	IsDragged = not IsDragged
...
function OpenCloakroomMenu()
	irpCore.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		title    = _U('cloakroom'),
		align    = 'bottom-right',
		elements = {
		...
		if data.current.value == 'citizen_wear' then
			irpCore.TriggerServerCallback('irp-skin:getPlayerSkin', function(skin, jobSkin)
...
function OpenVehicleSpawnerMenu(hospital, partNum)
	...
	irpCore.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle', {
		...
		if data.current.action == 'buy_vehicle' then
			...
			local authorizedVehicles = Config.AuthorizedVehicles[irpCore.PlayerData.job.grade_name]

			if #authorizedVehicles > 0 then
				for k,vehicle in ipairs(authorizedVehicles) do
					table.insert(shopElements, {
						label = ('%s - <span style="color:green;">%s</span>'):format(vehicle.label, _U('shop_item', irpCore.Math.GroupDigits(vehicle.price))),
						name  = vehicle.label,
						model = vehicle.model,
						price = vehicle.price,
						type  = 'car'
		...
		elseif data.current.action == 'garage' then
			...
			irpCore.TriggerServerCallback('irp-vehicleshop:retrieveJobVehicles', function(jobVehicles)
				if #jobVehicles > 0 then
					...
					irpCore.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_garage', {
						title    = _U('garage_title'),
						align    = 'bottom-right',
						elements = garage
						...
						if data2.current.stored then
							...
							if foundSpawn then
								...
								irpCore.Game.SpawnVehicle(data2.current.model, spawnPoint.coords, spawnPoint.heading, function(vehicle)
									irpCore.Game.SetVehicleProperties(vehicle, data2.current.vehicleProps)

									TriggerServerEvent('irp-vehicleshop:setJobVehicleState', data2.current.vehicleProps.plate, false)
...
function StoreNearbyVehicle(playerCoords)
	local vehicles, vehiclePlates = irpCore.Game.GetVehiclesInArea(playerCoords, 30.0), {}
		...
		for k,v in ipairs(vehicles) do
			...
			if GetVehicleNumberOfPassengers(v) == 0 and IsVehicleSeatFree(v, -1) then
				table.insert(vehiclePlates, {
					vehicle = v,
					plate = irpCore.Math.Trim(GetVehicleNumberPlateText(v))
				})
			end
		end
	else
	...
	irpCore.TriggerServerCallback('irp-ambulancejob:storeNearbyVehicle', function(storeSuccess, foundNum)
		if storeSuccess then
			local vehicleId = vehiclePlates[foundNum]
			local attempts = 0
			irpCore.Game.DeleteVehicle(vehicleId.vehicle)
			IsBusy = true
			...
			while DoesEntityExist(vehicleId.vehicle) do
				...
				vehicles = irpCore.Game.GetVehiclesInArea(playerCoords, 30.0)
				if #vehicles > 0 then
					for k,v in ipairs(vehicles) do
						if irpCore.Math.Trim(GetVehicleNumberPlateText(v)) == vehicleId.plate then
							irpCore.Game.DeleteVehicle(v)
							break
						end
					end
				end
...
function GetAvailableVehicleSpawnPoint(hospital, part, partNum)
	...
	for i=1, #spawnPoints, 1 do
		if irpCore.Game.IsSpawnPointClear(spawnPoints[i].coords, spawnPoints[i].radius) then
...
function OpenHelicopterSpawnerMenu(hospital, partNum)
	...
	irpCore.UI.Menu.Open('default', GetCurrentResourceName(), 'helicopter_spawner', {
		...
		if data.current.action == 'buy_helicopter' then
			...
			local authorizedHelicopters = Config.AuthorizedHelicopters[irpCore.PlayerData.job.grade_name]

			if #authorizedHelicopters > 0 then
				for k,helicopter in ipairs(authorizedHelicopters) do
					table.insert(shopElements, {
						label = ('%s - <span style="color:green;">%s</span>'):format(helicopter.label, _U('shop_item', irpCore.Math.GroupDigits(helicopter.price))),
		...
		elseif data.current.action == 'garage' then
			...
			irpCore.TriggerServerCallback('irp-vehicleshop:retrieveJobVehicles', function(jobVehicles)
				if #jobVehicles > 0 then
					...
					irpCore.UI.Menu.Open('default', GetCurrentResourceName(), 'helicopter_garage', {
...
function OpenShopMenu(elements, restoreCoords, shopCoords)
	...
	irpCore.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		title    = _U('vehicleshop_title'),
		align    = 'bottom-right',
		elements = elements
	}, function(data, menu)

		irpCore.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop_confirm', {
			...
			if data2.current.value == 'yes' then
				...
				local vehicle  = GetVehiclePedIsIn(playerPed, false)
				local props    = irpCore.Game.GetVehicleProperties(vehicle)
				props.plate    = newPlate

				irpCore.TriggerServerCallback('irp-ambulancejob:buyJobVehicle', function (bought)
					if bought then
						irpCore.ShowNotification(_U('vehicleshop_bought', data.current.name, irpCore.Math.GroupDigits(data.current.price)))
						...
						irpCore.Game.Teleport(playerPed, restoreCoords)
					else
		...
		irpCore.Game.SpawnLocalVehicle(data.current.model, shopCoords, 0.0, function(vehicle)
	...
	irpCore.Game.SpawnLocalVehicle(elements[1].model, shopCoords, 0.0, function(vehicle)
		table.insert(spawnedVehicles, vehicle)
		TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
		FreezeEntityPosition(vehicle, true)
	end)
...
function WarpPedInClosestVehicle(ped)
	...
	local vehicle, distance = irpCore.Game.GetClosestVehicle(coords)
...
RegisterNetEvent('irp-ambulancejob:heal')
AddEventHandler('irp-ambulancejob:heal', function(healType)
    local playerPed = PlayerPedId()
	...
	elseif healType == 'big' then
        ...
        SetEntityHealth(playerPed, newHealth)
        TriggerEvent('irp-hospital:client:RemoveBleed')
        TriggerEvent('irp-hospital:client:ResetLimbs')
    end
