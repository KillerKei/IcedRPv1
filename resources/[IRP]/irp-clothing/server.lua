...
TriggerEvent("irp:getSharedObject", function(library) 
	irpCore = library 
end)

RegisterServerEvent('irp-skin:save')
AddEventHandler('irp-skin:save', function()
    local src = source
    TriggerClientEvent('irp-clothing:guardarSkin',src)
end)

irpCore.RegisterServerCallback('irp-skin:getPlayerSkin', function(source, cb)
...
irpCore.RegisterServerCallback('irp-clothing:getSex', function(source, cb)
    local xPlayer = irpCore.GetPlayerFromId(source)
...
RegisterServerEvent("irp-clothing:insert_character_current")
AddEventHandler("irp-clothing:insert_character_current",function(data)
    if not data then return end
    local src = source
    local user = irpCore.GetPlayerFromId(src)
...
RegisterServerEvent("irp-clothing:insert_character_face")
AddEventHandler("irp-clothing:insert_character_face",function(data)
    if not data then return end
    local src = source
...
RegisterServerEvent("irp-clothing:get_character_face")
AddEventHandler("irp-clothing:get_character_face",function(pSrc)
    ...
    MySQL.Async.fetchAll("SELECT cc.model, cf.hairColor, cf.headBlend, cf.headOverlay, cf.headStructure FROM users cf INNER JOIN users cc on cc.identifier = cf.identifier WHERE cf.identifier = @identifier", {['identifier'] = characterId}, function(result)
        if (result ~= nil and result[1] ~= nil) then
            ...
            if model == 1885233650 or model == -1667301416 then
                TriggerClientEvent("irp-clothing:setpedfeatures", src, temp_data)
            end
        else
            TriggerClientEvent("irp-clothing:setpedfeatures", src, false)
        end
	end)
end)

RegisterServerEvent("irp-clothing:get_character_current")
AddEventHandler("irp-clothing:get_character_current",function(pSrc)
    local src = (not pSrc and source or pSrc)
    local user = irpCore.GetPlayerFromId(src)
    local characterId = user.identifier
    ...
    MySQL.Async.fetchAll("SELECT * FROM users WHERE identifier = @identifier", {['identifier'] = characterId}, function(result)
        ...
        TriggerClientEvent("irp-clothing:setclothes", src, temp_data,0)
	end)
end)

RegisterServerEvent("irp-clothing:retrieve_tats")
AddEventHandler("irp-clothing:retrieve_tats", function(pSrc)
    local src = (not pSrc and source or pSrc)
	local user = irpCore.GetPlayerFromId(src)
	MySQL.Async.fetchAll("SELECT * FROM playerstattoos WHERE identifier = @identifier", {['identifier'] = user.identifier}, function(result)
        if(#result == 1) then
			TriggerClientEvent("irp-clothing:settattoos", src, json.decode(result[1].tattoos))
		else
			local tattooValue = "{}"
			...
			TriggerClientEvent("irp-clothing:settattoos", src, {})
		end
	end)
end)

RegisterServerEvent("irp-clothing:set_tats")
AddEventHandler("irp-clothing:set_tats", function(tattoosList)
	local src = source
	local user = irpCore.GetPlayerFromId(src)
...
RegisterServerEvent("irp-clothing:get_outfit")
AddEventHandler("irp-clothing:get_outfit",function(slot)
    ...
    MySQL.Async.fetchAll("SELECT * FROM character_outfits WHERE steamid = @steamid and slot = @slot", {
        ...
        if result and result[1] then
            ...
            TriggerClientEvent("irp-clothing:setclothes", src, data,0)
...
RegisterServerEvent("irp-clothing:set_outfit")
AddEventHandler("irp-clothing:set_outfit",function(slot, name, data)
    if not slot then return end
    local src = source
...
RegisterServerEvent("irp-clothing:remove_outfit")
AddEventHandler("irp-clothing:remove_outfit",function(slot)
...
RegisterServerEvent("irp-clothing:list_outfits")
AddEventHandler("irp-clothing:list_outfits",function()
    local src = source
    local user = irpCore.GetPlayerFromId(src)
    local steamid = user.identifier
    local slot = slot
    ...
    MySQL.Async.fetchAll("SELECT slot, name FROM character_outfits WHERE steamid = @steamid", {['steamid'] = steamid}, function(skincheck)
    	TriggerClientEvent("irp-clothing:listOutfits",src, skincheck)
	end)
end)
        ...
        if result[1] == nil then
            MySQL.Async.fetchAll("SELECT * FROM users WHERE identifier = @identifier", {["identifier"] = steamid}, function(result)
                if result[1].skin then
                    TriggerClientEvent('irp-clothing:setclothes',src,{},json.decode(result[1].skin))
                else
                    TriggerClientEvent('irp-clothing:setclothes',src,{},nil)
                end
                return
            end)
        else
            TriggerEvent("irp-clothing:get_character_current", src)
        end
        TriggerClientEvent("irp-clothing:inService",src,isService)
    end)
end)
...
AddEventHandler("clothing:checkMoney", function(menu,askingPrice)
    local src = source
    local target = irpCore.GetPlayerFromId(src)
    ...
    if (tonumber(target.getMoney()) >= askingPrice) then
        ...
        TriggerClientEvent("irp-clothing:hasEnough",src,menu)
    else
