...
AddEventHandler('irp-skin:setLastSkin', function(skin)
    LastSkin = skin
  end)
...
function Save(save)
    if save then
        data = GetCurrentPed()
        TriggerServerEvent("irp-clothing:insert_character_current", data)
        if data.model == `mp_f_freemode_01` or data.model == `mp_m_freemode_01` then
            TriggerServerEvent("irp-clothing:insert_character_face", data)
            TriggerServerEvent("irp-clothing:set_tats", currentTats)
        end
    else
        LoadPed(oldPed)
    end
...
RegisterNetEvent("irp-clothing:guardarSkin")
AddEventHandler("irp-clothing:guardarSkin", function()
    data = GetCurrentPed()
    TriggerServerEvent("irp-clothing:insert_character_current", data)
    if data.model == `mp_f_freemode_01` or data.model == `mp_m_freemode_01` then
        TriggerServerEvent("irp-clothing:insert_character_face", data)
...
RegisterNetEvent("irp-clothing:inService")
AddEventHandler("irp-clothing:inService", function(service)
    isService = service
end)

RegisterNetEvent('irp-identity:identityCheck')
AddEventHandler('irp-identity:identityCheck', function(identityCheck)
	hasIdentity = identityCheck
end)

RegisterNetEvent("irp-clothing:hasEnough")
AddEventHandler("irp-clothing:hasEnough", function(menu)
...
RegisterNetEvent("irp-clothing:setclothes")
AddEventHandler("irp-clothing:setclothes", function(data,alreadyExist)
    player = GetPlayerPed(-1)
    local function setDefault()
        Citizen.CreateThread(function()
            firstChar = true
            local gender = nil
            irpCore.TriggerServerCallback('irp-clothing:getSex', function(sex)
				gender = sex
			end)
            Citizen.Wait(1000)
            if gender ~= "m" then
...
RegisterNetEvent("irp-clothing:defaultReset")
AddEventHandler("irp-clothing:defaultReset", function()
    local gender = nil
    irpCore.TriggerServerCallback('irp-clothing:getSex', function(sex)
        gender = sex
    end)
...
RegisterNetEvent("irp-clothing:settattoos")
AddEventHandler("irp-clothing:settattoos", function(playerTattoosList)
    currentTats = playerTattoosList
    SetTats(GetTats())
end)

RegisterNetEvent("irp-clothing:setpedfeatures")
AddEventHandler("irp-clothing:setpedfeatures", function(data)
    player = GetPlayerPed(-1)
    if data == false then
...
RegisterCommand("outfitadd", function(source, args, rawCommand)
    if exports["irp-housing"]:imClosesToRoom2() or exports["irp-apartments"]:imClosesToRoom3() or (IsNearShop(clothingShops) < 9.0) then
        if args[1] and args[2] then
            TriggerEvent('irp-clothing:outfits', 1, args[1], args[2])
        else
...
RegisterCommand("outfituse", function(source, args, rawCommand)
    if exports["irp-housing"]:imClosesToRoom2() or exports["irp-apartments"]:imClosesToRoom3() or (IsNearShop(clothingShops) < 9.0) then
        if args[1] then
            TriggerEvent('irp-clothing:outfits', 3, args[1])
        else
...
RegisterCommand("outfitdel", function(source, args, rawCommand)
    if exports["irp-housing"]:imClosesToRoom2() or exports["irp-apartments"]:imClosesToRoom3() or (IsNearShop(clothingShops) < 9.0) then
        if args[1] then
            TriggerEvent('irp-clothing:outfits', 2, args[1])
        else
            TriggerEvent('DoLongHudText', "You need to do something like /outfituse 1 | 1 being the slot id that you will have had previously saved", 1)  
        end
    else
...
RegisterCommand("outfits", function(source, args, rawCommand)
    if exports["irp-housing"]:imClosesToRoom2() or exports["irp-apartments"]:imClosesToRoom3() or (IsNearShop(clothingShops) < 9.0) then
        TriggerEvent('irp-clothing:outfits', 4)
    else
        TriggerEvent('DoLongHudText', "You are not near a wardrobe", 2)
    end
end, false)  

RegisterNetEvent('irp-clothing:listOutfits')
AddEventHandler('irp-clothing:listOutfits', function(skincheck)
...
RegisterNetEvent('irp-clothing:outfits')
AddEventHandler('irp-clothing:outfits', function(pAction, pId, pName)
    if pAction == 1 then
        TriggerServerEvent("irp-clothing:set_outfit",pId, pName, GetCurrentPed())
    elseif pAction == 2 then
        TriggerServerEvent("irp-clothing:remove_outfit",pId)
    elseif pAction == 3 then
        TriggerEvent('InteractSound_CL:PlayOnOne','Clothes1', 0.6)
        TriggerServerEvent("irp-clothing:get_outfit", pId)
    else
        TriggerServerEvent("irp-clothing:list_outfits")
    end
end)
