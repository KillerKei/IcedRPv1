...
AddEventHandler('irp:playerLoaded', function(xPlayer)
    PlayerData = xPlayer
	Citizen.Wait(20000)
	TriggerServerEvent("irp-vehshop:CheckFinanceStatus")
end)
			...
			if carDist <= Config.DrawTxtDist then
				...
				if (IsControlJustPressed(0, Config.KeyToBuyVeh) or IsDisabledControlJustPressed(0, Config.KeyToBuyVeh)) then
					if currentStock >= 1 then
							...
							if (carDist2 <= Config.DrawTxtDist and carModel2 == carModel) then
								...
								if not IsPedInAnyVehicle(player, true) and not BuyInProgress then
									...
									if (IsControlJustPressed(0, Config.KeyToConfirmBuyVeh) or IsDisabledControlJustPressed(0, Config.KeyToConfirmBuyVeh)) then
										irpCore.TriggerServerCallback('irp-vehshop:GetPlayerMoney', function(hasMoney, carPrice, carCommission, price, payment)
											BuyInProgress = true
											if hasMoney then
												local VehCoords = nil
												local Heading = 0
												...
												TriggerEvent('DoLongHudText', _U("veh_not_enough_money"),2)
												Citizen.Wait(250)
												BuyInProgress = false
											end
											Confirmed = true
									...
									elseif (IsControlJustPressed(0, Config.KeyToCancelBuyVeh) or IsDisabledControlJustPressed(0, Config.KeyToCancelBuyVeh)) then
										Citizen.Wait(500)
										TriggerEvent('DoLongHudText', _U('veh_purchase_cancel'))
										BuyInProgress = false
										Confirmed = true
									end
								end
						...
						TriggerEvent('DoLongHudText', _U("veh_not_in_stock"))
					end
					...
					if PlayerData.job ~= nil and PlayerData.job.name == Config.CarDealerJobLabel then
						...
						if carDist5 then
							...
							if currentCommission > (Config.MinCommission) then
								...
								TriggerServerEvent('irp-vehshop:DealerDiscountSV', currentModel, -1, currentID)
								exports["irp-taskbar"]:taskBar(500, "APPLYING CHANGES")
								Citizen.Wait(500)
							else
								ChangingValues = false
								TriggerEvent('DoLongHudText', _U("min_commission_limit"))
							end
						end
					end
					...
					if PlayerData.job ~= nil and PlayerData.job.name == Config.CarDealerJobLabel then
						...
						if carDist5 then
							...
							if currentCommission < (Config.MaxCommission) then
								...
								ChangingValues = false
								TriggerEvent('DoLongHudText', _U("max_commission_limit"))
							end
						end
					end
				...
				elseif (IsControlJustPressed(0, Config.KeyToFinanceVeh) or IsDisabledControlJustPressed(0, Config.KeyToFinanceVeh)) then
					if currentStock >= 1 then
							...
							if (carDist2 <= Config.DrawTxtDist and carModel2 == carModel) then
								...
								if not IsPedInAnyVehicle(player, true) and not FinanceInProgress then
									...
									if (IsControlJustPressed(0, Config.KeyToFinanceVeh) or IsDisabledControlJustPressed(0, Config.KeyToFinanceVeh)) then
										irpCore.TriggerServerCallback('irp-vehshop:GetFinancingMoney', function(hasUpfront, downPayment, carPrice, commission, payment)
						...
						TriggerEvent('DoLongHudText', _U("veh_not_in_stock"),2)
					end
					...
					if PlayerData.job ~= nil and PlayerData.job.name == Config.CarDealerJobLabel then
						...
						if carDist6 then
							...
							if currentDownpayment < (Config.MaxDownpayment) then
								...
								TriggerServerEvent('irp-vehshop:DealerDownpaymentSV', currentModel, 1, currentID)
								exports["irp-taskbar"]:taskBar(500, "APPLYING CHANGES")
								Citizen.Wait(500)
							else
								ChangingValues = false
								TriggerEvent('DoLongHudText', _U("max_downpayment_limit"))
							end
						end
					end
					...
					if PlayerData.job ~= nil and PlayerData.job.name == Config.CarDealerJobLabel then
						...
						if carDist6 then
							...
							if currentDownpayment > (Config.MinDownpayment) then
								...
								ChangingValues = false
								TriggerEvent('DoLongHudText', _U("min_downpayment_limit"))
							end
						end
					end
...
function TestDriveVehicleFunction(currentModel, currentID)
	if TestVehicle == nil and not TestDriving then
		local player = GetPlayerPed(-1)
		TriggerEvent('DoLongHudText', _U("test_veh_spawned"))
		TestDriving = true
		local VehCoords = nil
		local Heading = 0
		if currentID ~= 9 then
		...
		TriggerEvent('DoLongHudText', _U("already_test_driving"))
	end
end
				...
				if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, Pos[1], Pos[2], Pos[3], false) <= Mk.DrawDist then
					...
					if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, Pos[1], Pos[2], Pos[3], false) <= v.DrawText then
						...
						if IsControlJustPressed(0, v.Key) then
							...
							TriggerEvent('DoLongHudText', _U("test_veh_returned"))
							Citizen.Wait(1500)
							irpCore.Game.DeleteVehicle(TestVehicle)
							TestDriving = false
							TestVehicle = nil
							TestID = 0
						end
...
function OpenReplaceMainMenu(currentID)
	...
	for k,v in pairs(categories) do
		...
		elseif currentID == 7 then
			if v.name == "suvs" or v.name == "utility" or v.name == "offroad" or v.name == "vans" then
				table.insert(elements,{ label = v.label, name = v.name})
			end
		else
	...
	irpCore.UI.Menu.Open('default', GetCurrentResourceName(), "cardealer_replace_menu",
		{
...
function OpenCategoryMenu(selectedCategory, currentID)
	...
	irpCore.UI.Menu.Open('default', GetCurrentResourceName(), "cardealer_category_menu",
		{
		...
		function(data, menu)
			...
			menuOpen = false
			irpCore.UI.Menu.CloseAll()
			TriggerServerEvent('irp-vehshop:ReplaceVehSV', data.current.model, data.current.name, currentID)
		end, function(data, menu)
		menu.close()
		menuOpen = false
		irpCore.UI.Menu.CloseAll()
...
function OpenShopMainMenu()
	...
	irpCore.TriggerServerCallback('irp-vehshop:GetDealerCount', function(DealerOnline)
		...
		irpCore.UI.Menu.Open('default', GetCurrentResourceName(), "cardealer_shop_menu",
			{
...
function OpenVehicleSaleMenu()
	irpCore.TriggerServerCallback('irp-vehshop:GetAllOwnedVehicles', function(results)
		...
		exports["irp-taskbar"]:taskBar(1000, _U('progbar_retrieve_veh'))
		Citizen.Wait(1000)
				
		irpCore.UI.Menu.Open('default', GetCurrentResourceName(), "cardealer_sell_owned_vehicles",
			{
		...
		function(data, menu)
			...
			-- Confirm Menu:
			irpCore.UI.Menu.Open('default', GetCurrentResourceName(), "cardealer_veh_sale_confirm",
				{
				...
				if data.current.value == "yes" then
					...
					irpCore.TriggerServerCallback('irp-vehshop:SellOwnedVehicle', function(sold, sellPrice, plate, model)
						if sold then
							TriggerEvent('DoLongHudText', 'You received $'..math.floor(sellPrice)..' for selling your vehicle')
							...
							if DelVehPlate == plate then
								...
								TriggerEvent('DoLongHudText', _U('shop_sell_veh_taken'))
							end
						end
...
function OpenShopBillingMenu()
	irpCore.TriggerServerCallback('irp-vehshop:GetAllOwnedVehicles', function(results)
		...
		exports["irp-taskbar"]:taskBar(1000, _U('progbar_retrieve_bills'))
		Citizen.Wait(1000)
		
		irpCore.UI.Menu.Open('default', GetCurrentResourceName(), "cardealer_financing_bills",
			{
...
function PayFinancialBill(plate,price,model,finance,repaytime)	
	...
	irpCore.UI.Menu.Open('dialog', GetCurrentResourceName(), 'shop_repayment_amount', {
	...
	function(data, menu)
		...
		if amount < difference then
			TriggerEvent('DoLongHudText', _U('min_repayment', math.floor(difference)))
			...
			irpCore.TriggerServerCallback('irp-vehshop:RepayAmount', function(hasPaid) 
				if hasPaid then 
					TriggerEvent('DoLongHudText', _U('paid_fin_bill', math.floor(amount)))
				else 
					TriggerEvent('DoLongHudText', _U('not_enough_money'),2)
				end
			end, vehPlate, amount)
...
function OpenShopCatalogMenu()
	...
	irpCore.UI.Menu.Open('default', GetCurrentResourceName(), "cardealer_shop_categories",
		{
...
function OpenShopCategoryMenu(category)
	...
	irpCore.UI.Menu.Open('default', GetCurrentResourceName(), "cardealer_shop_vehicle_list",
		{
...
function BuyShopMenuCar(buyVeh, vehGroup, category)
	irpCore.UI.Menu.Open('default', GetCurrentResourceName(), "cardealer_shop_confirm",
		{
		...
		if data.current.value == "yes" then
			...
			irpCore.TriggerServerCallback('irp-vehshop:ShopGetPlyMoney', function(gotMoney, price, payment)
				if gotMoney then
					local VehCoords = nil
					local Heading = 0
					for r,t in pairs(Config.PurchasedVehSpawn) do
					...
					Citizen.Wait(250)
					irpCore.Game.SpawnVehicle(vehModel,VehCoords,Heading, function(purchCar)
						Citizen.Wait(10)
						...
						local vehProps = irpCore.Game.GetVehicleProperties(purchCar)
						vehProps.plate = numberPlate
						SetVehicleNumberPlateText(purchCar, numberPlate)
						TriggerServerEvent('garage:addKeys', numberPlate)
						TriggerEvent('DoLongHudText', 'You received keys for this vehicle.', 1)
						-- end
						TriggerServerEvent('irp-vehshop:ShopBuyAddCarToDB', vehProps, price, payment, vehModel)
						GetCarDealerData()
						TriggerEvent('DoLongHudText', _U('veh_purchased'))
					end)										
				else
					TriggerEvent('DoLongHudText', _U("veh_not_enough_money"))
				end
...
function OpenBossMenu()
	...
	irpCore.UI.Menu.Open('default', GetCurrentResourceName(), "cardealer_boss_menu",
		{
...
function OpenEmployeesMenu(jobNameDB)
	...
	irpCore.UI.Menu.Open('default', GetCurrentResourceName(), "cardealer_employee_menu",
		{
	...
	function(data, menu)
		...
		if data.current.value == "open_employee_list" then
			irpCore.TriggerServerCallback('irp-vehshop:getEmployees', function(employees)
				local elements = {
				...
				irpCore.UI.Menu.Open('list', GetCurrentResourceName(), 'cardealer_employee_list_' .. jobNameDB, elements, function(data2, menu2)
					local employee = data2.data
					-- PROMOTE MENU:
					if data2.value == 'promote' then
						menu2.close()
						irpCore.TriggerServerCallback('irp-vehshop:getJob', function(job)
							local elements = {}
							...
							irpCore.UI.Menu.Open('default', GetCurrentResourceName(), 'cardealer_promote_employee_' .. jobNameDB, {
								title    = _U('promote_employee', employee.name),
								align    = 'center',
								elements = elements
							}, function(data3, menu3)
								menu3.close()
								TriggerEvent('DoLongHudText', _U('you_have_promoted', employee.name, data3.current.label))

								irpCore.TriggerServerCallback('irp-vehshop:setJob', function()
									OpenEmployeesMenu(jobNameDB)
					...
					elseif data2.value == 'fire' then
						TriggerEvent('DoLongHudText', _U('you_have_fired', employee.name))
		...
		elseif data.current.value == "open_reqruit_menu" then
			irpCore.TriggerServerCallback('irp-vehshop:getOnlinePlayers', function(players)
				local elements = {}
				...
				irpCore.UI.Menu.Open('default', GetCurrentResourceName(), 'cardealer_recruit_' .. jobNameDB, {
					title    = _U('recruiting'),
					align    = 'center',
					elements = elements
				}, function(data2, menu2)
					-- YES / NO OPTION:
					irpCore.UI.Menu.Open('default', GetCurrentResourceName(), 'cardealer_recruit_confirm_' .. jobNameDB, {
						...
						if data3.current.value == 'yes' then
							TriggerEvent('DoLongHudText', _U('you_have_hired', data2.current.name))
...
function OpenAccountsMenu()
	...
	irpCore.TriggerServerCallback('irp-vehshop:GetAccountMoney', function(accountMoney,moneyOnPlayer)
		
		irpCore.UI.Menu.Open('default', GetCurrentResourceName(), "cardealer_accounts_menu",
			{
...
function OpenAccountsDialog(option, label, accountMoney, moneyOnPlayer)
	...
	irpCore.UI.Menu.Open('dialog', GetCurrentResourceName(), 'cardealer_accounts_dialog', {
		title    = text
	}, function(data, menu)
		local amount = tonumber(data.value)
		local callback, msg, progbar = '', '', ''
		if amount == nil or amount == '' then
			TriggerEvent('DoLongHudText', _U('invalid_amount'),2)
		else
			if option == 1 then
				callback = 'Withdraw'
			...
			irpCore.TriggerServerCallback('irp-vehshop:Accounts'..callback..'', function(approved)
				if approved then
					menu.close()
					exports["irp-taskbar"]:taskBar(2000, progbar)
					Citizen.Wait(2000)
					ShowNotifyESX(msg)
					OpenAccountsMenu()
				else
...
-- REGISTRATION PAPER:
local open = false
RegisterNetEvent('irp-vehshop:openRegCL')
AddEventHandler('irp-vehshop:openRegCL', function(data, vehPlate)
	open = true
...
RegisterNetEvent('irp-vehshop:ReplaceVehCL')
AddEventHandler('irp-vehshop:ReplaceVehCL', function(car, currentID)
    ReplaceVehicle(car, currentID)
end)

-- Server called event to get car dealer data:
RegisterNetEvent('irp-vehshop:DealerDiscountCL')
AddEventHandler('irp-vehshop:DealerDiscountCL', function()
    GetCarDealerData()
    ChangingValues = false
end)

    -- Server called event to get car dealer data:
RegisterNetEvent('irp-vehshop:DealerDownpaymentCL')
AddEventHandler('irp-vehshop:DealerDownpaymentCL', function()
    GetCarDealerData()
    ChangingValues = false
end)

    -- Server called event to get car dealer data:
RegisterNetEvent('irp-vehshop:UpdateCarDealerStockCL')
AddEventHandler('irp-vehshop:UpdateCarDealerStockCL', function(vehModel,vehPlate)
    ...
    irpCore.TriggerServerCallback('irp-vehshop:UpdateDealerStock', function(stockUpdated)
...
function GetCarDealerData()
    irpCore.TriggerServerCallback("irp-vehshop:FetchData", function(data1, data2, data3)
        vehicles = data1
        display = data2
        categories = data3
    end)
...
function SpawnDisplayVeh()
    ...
    irpCore.Game.SpawnLocalVehicle(hash, syncPos, v.Heading, function(carDisplay)
        Citizen.Wait(50)
...
function ReplaceVehicle(car, currentID)
        ...
        irpCore.Game.DeleteVehicle(DisplayCars[currentID])
        ...
        irpCore.Game.SpawnLocalVehicle(hash, VehCoords, VehHeading, function(newCar)
            Citizen.Wait(50)
...
function SpawnPurchasedVehicle(currentModel, VehCoords, Heading, player, carPrice, carCommission, price, payment)
    irpCore.Game.SpawnVehicle(currentModel,VehCoords,Heading, function(boughtCar)
        Citizen.Wait(10)
        ...
        -- Generate Plate:
        local numberPlate = ProduceNumberPlate()
        local vehProps = irpCore.Game.GetVehicleProperties(boughtCar)
        vehProps.plate = numberPlate
        ...
        -- end
        GetCarDealerData()
        TriggerServerEvent('irp-vehshop:AddVehToDatabase', vehProps, carPrice, carCommission, price, payment, currentModel)
        TriggerEvent('DoLongHudText', _U('veh_purchased'))
        BuyInProgress = false
    end)
end
...
function SpawnFinancedVehicle(currentModel, VehCoords, Heading, player, downPayment, carPrice, commission, payment)
    irpCore.Game.SpawnVehicle(currentModel,VehCoords,Heading, function(financedCar)
        Citizen.Wait(10)
        ...
        -- Generate Plate:
        local numberPlate = ProduceNumberPlate()
        local vehProps = irpCore.Game.GetVehicleProperties(financedCar)
        vehProps.plate = numberPlate
        ...
        -- end
        GetCarDealerData()
        TriggerServerEvent('irp-vehshop:AddFinancedVehToDB', vehProps, downPayment, carPrice, commission, payment, currentModel)
        TriggerEvent('DoLongHudText', _U('veh_purchased'))
        FinanceInProgress = false
    end)
end
...
function SpawnTestDriveVeh(currentModel, VehCoords, Heading)
    irpCore.Game.SpawnVehicle(currentModel, VehCoords, Heading, function(testVeh)
        Citizen.Wait(10)
		...
		TriggerServerEvent('garage:addKeys', numberPlate)
		TriggerEvent('DoLongHudText', 'You received keys for this vehicle.', 1)
		SetVehicleNumberPlateText(testVeh, numberPlate)
        TestVehicle = testVeh
        TestID = currentID
    end)
end
