...
AddEventHandler("krp:playerBecameJob", function(jobpassed, name, notify)
	job = jobpassed
    if not job then
        job = "None"
    end
	...
	while true do
		...
		if IsPedShooting(ped) then
			...
			if throwableWeapons[weapon] then
				if exports["irp-inventory"]:hasEnoughOfItem(weapon,1,false) then
		...
		if IsPedPlantingBomb(ped) then
			if exports["irp-inventory"]:hasEnoughOfItem("741814745",1,false) then
...
function unholster1h(weaponHash)
	...
	local anim = "intro"
	local myJob = irpCore.PlayerData.job.name
	local ped = PlayerPedId()
