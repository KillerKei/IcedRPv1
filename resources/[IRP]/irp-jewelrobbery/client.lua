...
RegisterNetEvent('irp-jewelrobbery:policeclosure')
AddEventHandler('irp-jewelrobbery:policeclosure', function()
	policeclosed = true
	storeclosed = false
	IsAbleToRob = false
end)
...
AddEventHandler('irp:playerLoaded', function(xPlayer)
	PlayerData = xPlayer
	TriggerServerEvent('irp-jewelrobbery:loadconfig')
end)

RegisterNetEvent('irp-jewelrobbery:resetcases')
AddEventHandler('irp-jewelrobbery:resetcases', function(list)
...
RegisterNetEvent('irp-jewelrobbery:setcase')
...
RegisterNetEvent('irp-jewelrobbery:loadconfig')
AddEventHandler('irp-jewelrobbery:loadconfig', function(casestatus)
...
RegisterNetEvent('irp-jewelrobbery:playsound')
...
AddEventHandler('irp-jewelrobbery:EnteredArea', function()
	for i, v in pairs(Config.CaseLocations) do
		if v.Broken then
			CreateModelSwap(v.Pos.x, v.Pos.y, v.Pos.z, 0.1, GetHashKey(v.Prop1), GetHashKey(v.Prop), false )
		end
...
AddEventHandler('irp-jewelrobbery:LeftArea', function()
	for i, v in pairs(Config.CaseLocations) do
		if v.Broken then
			RemoveModelSwap(v.Pos.x, v.Pos.y, v.Pos.z, 0.1, GetHashKey(v.Prop1), GetHashKey(v.Prop), false )
		end
...
Citizen.CreateThread( function()
	while true do 
		...
		if IsInArea and not HasAlreadyEnteredArea then
			TriggerEvent('irp-jewelrobbery:EnteredArea')
			shockingevent = false
		...
		if not IsInArea and HasAlreadyEnteredArea then
			TriggerEvent('irp-jewelrobbery:LeftArea')
			HasAlreadyEnteredArea = false
			shockingevent = false
			IsAbleToRob = false
			storeclosed = false
...
function CheckPolice()
	if CopsOnline ~= exports["irp-jobnumbers"]:jobonline('cops') then
		HasAlreadyEnteredArea = false
	end
	CopsOnline = exports["irp-jobnumbers"]:jobonline('cops')
	return exports["irp-jobnumbers"]:jobonline('cops')
end
						...
						if IsControlJustPressed(0, 38) and not IsBusy and not IsPedWalking(ply) and not IsPedRunning(ply) and not IsPedSprinting(ply) then
							...
							if breakchance <= robalbe.chance then
								if policenotify <= Config.PoliceNotifyBroken and not HasNotified then
									local playerCoords = GetEntityCoords(PlayerPedId())
									TriggerEvent('irp-dispatch:jewelrobbery')
									HasNotified = true
								end
								Citizen.Wait(2100)
								TriggerServerEvent('irp-jewelrobbery:playsound', v.Pos.x, v.Pos.y, v.Pos.z, 'break')
								CreateModelSwap(v.Pos.x, v.Pos.y, v.Pos.z,  0.1, GetHashKey(v.Prop1), GetHashKey(v.Prop), false )
								ClearPedTasksImmediately(ply)
								TriggerServerEvent("irp-jewelrobbery:setcase", i, true)	
							else
								Citizen.Wait(2100)
								TriggerServerEvent('irp-jewelrobbery:playsound', v.Pos.x, v.Pos.y, v.Pos.z, 'nonbreak')
								ClearPedTasksImmediately(ply)
        	...
        	Wait(1)
			 ...
			 if IsControlJustReleased(0,38) and distance < 1.0 then
				if CheckPolice() >= Config.MinPolice then
             		if exports["irp-inventory"]:hasEnoughOfItem("Gruppe6Card3",1,false) then
						TriggerEvent("inventory:removeItem", "Gruppe6Card3", 1)
						TriggerServerEvent("irp-doors:alterlockstate",61)
						TriggerServerEvent("irp-doors:alterlockstate",62)
					 end
             	end
             end
		else
