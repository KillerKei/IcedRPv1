...
function LeaveBed()
    ...
    TriggerServerEvent('irp-hospital:server:LeaveBed', bedOccupying)
...
RegisterNetEvent('irp-hospital:client:RPCheckPos')
AddEventHandler('irp-hospital:client:RPCheckPos', function()
    TriggerServerEvent('irp-hospital:server:RPRequestBed', GetEntityCoords(PlayerPedId()))
end)

RegisterNetEvent('irp-hospital:client:RPSendToBed')
AddEventHandler('irp-hospital:client:RPSendToBed', function(id, data)
    bedOccupying = id
...
RegisterNetEvent('irp-hospital:client:SendToBed')
AddEventHandler('irp-hospital:client:SendToBed', function(id, data)
    bedOccupying = id
    bedOccupyingData = data
    ...
    Citizen.CreateThread(function ()
        ...
        TriggerServerEvent('irp-hospital:server:EnteredBed')
    end)
end)

RegisterNetEvent('irp-hospital:client:FinishServices')
AddEventHandler('irp-hospital:client:FinishServices', function()
                ...
                if hspDist < 3 then
                    ...
                    if IsControlJustReleased(0, 54) then
                        if (GetEntityHealth(PlayerPedId()) < 201) or (IsInjuredOrBleeding()) then
                            ...
                            exports["irp-taskbar"]:taskBar(1500, "Checking Credentials")
                                TriggerEvent('irp-ambulancejob:revive')
                                ClearPedTasks(PlayerPedId())
                                Citizen.Wait(1000)
                                TriggerServerEvent('irp-hospital:server:RequestBed')
                            else
