...
function CheckDamage(ped, bone, weapon)
    ...
    if Config.Bones[bone] ~= nil then
        if not BodyParts[Config.Bones[bone]].isDamaged then
            ...
            TriggerServerEvent('irp-hospital:server:SyncInjuries', {
                limbs = BodyParts,
...
RegisterNetEvent('irp-hospital:client:SyncBleed')
AddEventHandler('irp-hospital:client:SyncBleed', function(bleedStatus)
    isBleeding = tonumber(bleedStatus)
    DoBleedAlert()
end)

RegisterNetEvent('irp-hospital:client:FieldTreatLimbs')
AddEventHandler('irp-hospital:client:FieldTreatLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 1
    end
...
RegisterNetEvent('irp-hospital:client:FieldTreatBleed')
AddEventHandler('irp-hospital:client:FieldTreatBleed', function()
    if isBleeding > 1 then
...
RegisterNetEvent('irp-hospital:client:ReduceBleed')
AddEventHandler('irp-hospital:client:ReduceBleed', function()
    if isBleeding > 0 then
        isBleeding = tonumber(isBleeding) - 1
...
RegisterNetEvent('irp-hospital:client:ResetLimbs')
AddEventHandler('irp-hospital:client:ResetLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 0
    end
...
RegisterNetEvent('irp-hospital:client:RemoveBleed')
AddEventHandler('irp-hospital:client:RemoveBleed', function()
    isBleeding = 0
...
RegisterNetEvent('irp-hospital:client:UsePainKiller')
AddEventHandler('irp-hospital:client:UsePainKiller', function(tier)
    if tier < 4 then
...
RegisterNetEvent('irp-hospital:client:UseAdrenaline')
AddEventHandler('irp-hospital:client:UseAdrenaline', function(tier)
    if tier < 4 then
        onDrugs = 180 * tier
    end
