...
RegisterServerEvent('irp-hospital:server:RequestBed')
AddEventHandler('irp-hospital:server:RequestBed', function()
    for k, v in pairs(beds) do
        if not v.taken then
            v.taken = true
            bedsTaken[source] = k
            TriggerClientEvent('irp-hospital:client:SendToBed', source, k, v)
            return
        end
    end
...
RegisterServerEvent('irp-hospital:server:RPRequestBed')
AddEventHandler('irp-hospital:server:RPRequestBed', function(plyCoords)
    ...
    for k, v in pairs(beds) do
        ...
        if distance < 3.0 then
            if not v.taken then
                v.taken = true
                foundbed = true
                TriggerClientEvent('irp-hospital:client:RPSendToBed', source, k, v)
                return
            else
...
RegisterServerEvent('irp-hospital:server:EnteredBed')
AddEventHandler('irp-hospital:server:EnteredBed', function()
    local src = source
    ...
    TriggerClientEvent('irp-hospital:client:FinishServices', src)
end)

RegisterServerEvent('irp-hospital:server:LeaveBed')
AddEventHandler('irp-hospital:server:LeaveBed', function(id)
    beds[id].taken = false
end)