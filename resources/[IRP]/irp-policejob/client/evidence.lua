...
-- RegisterCommand('denomstest', function()
-- 	--TriggerEvent('denoms', 100)
-- 	TriggerEvent('chatMessage', '', 9, 'test')
-- end)
...
function bleedMulti(WeaponValue)
    ...
    print("current blood loss = " .. bleedmulti)
end
...
AddEventHandler('eventConc', function(level)
	print("THIS IS DOING SCREEN DAMAGE 2")
	if math.random(100) > 40 then
		...
		if not fullcol then
			...
			Wait(100)
			print("THIS IS DOING SCREEN DAMAGE 8")
...
AddEventHandler("DoDamageFunction",function(InjuryType,CurrentHitCount,boneIndex)
	print("THIS IS DOING SCREEN DAMAGE")
...
function DoScreenFade(length,todark)
	print("THIS IS DOING SCREEN DAMAGE 3")
	if passingout then
		return
	end
...
function redflash(enable)
	if enable then
		print("THIS IS DOING SCREEN DAMAGE 5")
        StartScreenEffect("SuccessTrevor", 1.0, 0)
        StopScreenEffect("SuccessTrevor")   
        Citizen.Wait(50)     
        StartScreenEffect("SuccessTrevor", 1.0, 0)
	else
		print("THIS IS DOING SCREEN DAMAGE 6")
        StartScreenEffect("SuccessTrevor", 1.0, 0)
        StopScreenEffect("SuccessTrevor")
    end
end
