...
AddEventHandler('police:gsr', function(t)
	...
	local finished = exports["irp-taskbar"]:taskBar(15000,"GSR Testing")
    if finished == 100 then
		t, distance = GetClosestPlayer()
		if(distance ~= -1 and distance < 7) then
			TriggerServerEvent("irp-policejob:checkgsr", GetPlayerServerId(t))
		end
	end
end)
...
AddEventHandler('police:getArrested2', function(cuffer)
	...
	local finished = 0
	if not exports["irp-ambulancejob"]:GetDeath() then
		finished = exports["irp-taskbarskill"]:taskBar(1200,7)
	end
...
AddEventHandler( 'Police:Radio', function()
	if isCop then
	    ...
	    if ( DoesEntityExist( ped ) and not IsEntityDead( ped )) and exports["irp-ambulancejob"]:GetDeath() ~= 1 then
	...
	if t ~= nil and t ~= -1 then
		if(distance ~= -1 and distance < 5) then
			...
			if myjob ~= "ambulance" then
				local bandages = exports["irp-inventory"]:getQuantity("bandage")
				if bandages == 0 then
					return
				else
...
AddEventHandler("police:rob", function()
	if not exports["irp-ambulancejob"]:GetDeath() then
		...
		local finished = exports["irp-taskbar"]:taskBar(3500,"Robbing",false,true)	
...
AddEventHandler( 'KneelHU', function()
	...
	if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
		...
		local finished = exports["irp-taskbar"]:taskBar(2500,"Surrendering")					
...
AddEventHandler('FlipVehicle', function()
	local finished = exports["irp-taskbar"]:taskBar(5000,"Flipping Vehicle Over",false,true)	
...
function deleteVeh(ent)
	...
	local finished = exports["irp-taskbar"]:taskBar(1000,"Impounding",false,true)	
