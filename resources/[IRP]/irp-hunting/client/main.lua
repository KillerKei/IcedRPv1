...
AddEventHandler('irp:playerLoaded', function(xPlayer)
	PlayerData = xPlayer
	--print("Caught player loaded for hunting")
	ScriptLoaded()
end)
						...
						if distance < 1.0 then
							if IsControlJustReleased(0, Keys['E']) then
								if OnGoingHuntSession then
									...
									if irpCore.GetPlayerData().money >= 1000 then
										TriggerServerEvent('irp:huntingdeposit')
										StartHuntingSession()
									elseif irpCore.GetPlayerData().money < 1000 then
...
function StartHuntingSession()
	if canHunt == true then
		...
		if OnGoingHuntSession then
			...
			if exports["irp-inventory"]:getQuantity(100416529) then
				TriggerServerEvent('irp-hunting:removeloadout')
				...
				TriggerServerEvent('irp:huntingreturnree')
				--print("Started del")
				StartDelay()
				...
				--Car
				RandomLoc()
				irpCore.Game.SpawnVehicle('blazer', vector3( -769.63067626953, 5592.7573242188, 33.48571395874), 169.79, function(vehicle)
				HuntCar = vehicle
				local plate = GetVehicleNumberPlateText(HuntCar)
				TriggerServerEvent('garage:addKeys', plate)
				TriggerServerEvent('irp-hunting:giveloadout')
									...
									if IsControlJustReleased(0, Keys['E']) then
										if GetSelectedPedWeapon(PlayerPedId()) == GetHashKey('WEAPON_KNIFE')  then
											if DoesEntityExist(value.id) then
												...
												local finished = exports["irp-taskbar"]:taskBar(10000,"Skinning")
												TaskPlayAnim(PlayerPedId(), "amb@medic@standing@kneel@base" ,"base" ,8.0, -8.0, -1, 1, 0, false, false, false )
												TaskPlayAnim(PlayerPedId(), "anim@gangops@facility@servers@bodysearch@" ,"player_search" ,8.0, -8.0, -1, 48, 0, false, false, false )
    ...
    function()
        while true do
            ...
            if distance < 5.0 then
                ...
                if distance < 1.0 then
					if IsControlJustReleased(0, Keys["E"]) then
						if exports["irp-inventory"]:getQuantity("deer") >= 2 then
								playerAnim()
								local finished = exports["irp-taskbar"]:taskBar(4000,"Selling Goods",true,false,playerVeh)
								if finished == 100 then
									ClearPedTasksImmediately(PlayerPedId())
									TriggerEvent('inventory:removeItem', 'deer', 2)
									Citizen.Wait(1000)
									TriggerEvent('loopUpdateItems')
									TriggerServerEvent('irp-hunting:sell')
							end
                        end
                    end
                end
